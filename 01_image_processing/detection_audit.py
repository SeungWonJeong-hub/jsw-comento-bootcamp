"""
1차 업무 [피드백 반영 2] - 정답 마스크가 없을 때 검출 품질을 표본조사로 추정한다.

기존 README 는 "정답 마스크가 없으므로 이 수치가 얼마나 정확한지 판정할 수 없다"
로 끝났다. 판정할 수 없는 것이 아니라, 전수 채점을 못 할 뿐이다. 표본을 뽑아
사람이 판정하면 전체 정확도를 구간으로 추정할 수 있다.

대상이 이미지 1장(2131x1421)이므로 "무작위 100장"을 "무작위 100개 패치"로 옮긴다.

  1. 층화 표본  검출된 화소에서 50개, 검출 안 된 화소에서 50개를 뽑는다.
                전체에서 균등 추출하면 염전이 화면의 0.88% 뿐이라 100개 중
                한 개 남짓만 양성이라 놓친 화소를 찾을 수가 없다.
  2. 판정        패치 격자 이미지를 보고 CSV 의 verdict 열에 o / x 를 적는다.
  3. 추정        정밀도와 놓침률을 윌슨 신뢰구간과 함께 계산한다.

  --strategy uncertain 은 능동학습(active learning) 방식이다. HSV 범위 경계에
  가장 가까운, 즉 조금만 기준이 달랐어도 판정이 뒤집혔을 화소를 우선 뽑는다.
  같은 100개를 판정해도 기준선을 어디에 그어야 할지에 대해 더 많이 알게 된다.
...
def hsv_margin(image: np.ndarray, color: str) -> np.ndarray:
    """화소마다 HSV 범위 경계까지의 여유를 부호 있는 값으로 계산한다.

    양수면 범위 안쪽(검출됨)이고 값이 클수록 확실하다. 음수면 바깥쪽이고
    절댓값이 클수록 확실히 아니다. 0 근처가 판정이 아슬아슬한 화소다.

    채널마다 단위가 달라(H 0~179, S/V 0~255) 각 범위의 절반 폭으로 나눠
    무차원으로 만든 뒤 최솟값을 취한다. 세 조건을 모두 만족해야 검출이므로
    가장 빠듯한 채널이 그 화소의 여유가 된다.
    """
    ...
    for lower, upper in COLOR_RANGES[color]:
        ...
        center = (lo + hi) / 2.0
        # 중심에서 떨어진 거리를 반폭으로 정규화하면 1 이 경계다.
...
def sample_points(mask: np.ndarray, margin: np.ndarray, count: int,
    ...
    """검출/미검출 두 층에서 절반씩 뽑는다.

    strategy
      random     각 층에서 균등 추출. 정밀도의 불편추정치를 얻는 정공법이다.
      uncertain  각 층에서 경계에 가까운 화소를 우선 추출(능동학습). 추정값은
                 전체를 대표하지 않지만, 기준선을 어디로 옮겨야 할지 알려준다.
    """
    half = PATCH // 2
    h, w = mask.shape
    # 패치를 온전히 잘라낼 수 있는 안쪽 영역만 후보로 둔다.
    ...
    for name, area in strata.items():
        ...
        if strategy == "uncertain":
            ...
            # 상위 20배수 안에서 무작위로 뽑아 한 덩어리에 몰리지 않게 한다.
...
def build_sheet(image: np.ndarray, points: list[dict]) -> np.ndarray:
    """패치를 번호와 함께 격자로 붙인다. 가운데 십자가 판정 대상 화소다."""
    half = PATCH // 2
    tiles = []
    for p in points:
        ...
        # 판정 대상은 패치 전체가 아니라 정중앙 화소 하나다. 십자로 짚어 준다.
        c = TILE // 2
        ...
        # 층을 색으로 흘리지 않기 위해 테두리는 모두 같은 회색으로 둔다.
...
def write_csv(points: list[dict], path: Path, strategy: str) -> None:
    ...
    verdict 에는 '검출이 맞았는가'가 아니라 '이 화소가 실제로 대상인가'를 적는다
    (o = 염전이다, x = 아니다). 판정자가 검출 결과를 모르는 채로 봐야 편향이
    안 생기므로, 층 정보를 몰라도 답할 수 있는 질문이어야 한다.
    """
...
def wilson(successes: int, total: int, z: float = 1.96) -> tuple[float, float, float]:
    """윌슨 점수 구간. 표본이 작거나 비율이 0/1 에 붙어도 무너지지 않는다.

    단순한 정규 근사(p +- z*sqrt(p(1-p)/n))는 100개 표본에서 구간이 0 아래로
    내려가거나 1 을 넘는 일이 흔해 쓸 수 없다.
    """
    if total == 0:
...
def score(path: Path) -> None:
    """사람이 채운 CSV 를 읽어 정밀도와 놓침률을 구간추정한다."""
    ...
    if strategy == "uncertain":
        ...
        print("        아니다. '기준선 부근이 얼마나 위태로운가'를 재는 수치다.")
    if not judged:
    ...
    # 경계 근처에서 판정이 갈리는 지점을 찾으면 임계값을 어디로 옮길지 보인다.
    ...
    if wrong and right and strategy == "uncertain":
        # 표본 자체를 여유값으로 골랐으므로 여유의 대소를 비교해도 의미가 없다.
        print("  (여유값 비교는 random 표본에서만 뜻이 있다)")
    elif wrong and right:
        mw, mr = float(np.median(wrong)), float(np.median(right))
        print(f"  오검출 화소의 여유 중앙값 {mw:+.3f} / 정검출 {mr:+.3f}")
        if mw < mr:
            print("   -> 오검출이 경계 근처에 몰려 있다. 범위를 좁히면 정밀도가 오른다.")
        else:
            print("   -> 오검출이 경계가 아니라 범위 한가운데에 있다. 범위를 좁혀도")
            print("      정검출이 함께 잘려 나가므로, HSV 범위 조정으로는 못 고친다.")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--score", type=Path, help="채워진 CSV 를 채점만 한다")
    return p.parse_args()
...
def main() -> None:
    ...
    # 판정 시트는 위성 사진을 잘라 붙인 것이라 사진 계열이다. 저장소의 다른
    # 결과물과 같은 규칙으로 JPEG 에 담는다 (무손실 PNG 로는 6배 커진다).
