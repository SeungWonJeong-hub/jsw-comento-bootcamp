"""스테레오 삼각측량으로 깊이 맵을 만들고 3D 포인트 클라우드로 변환합니다.
...
두 번째 시점을 어떻게 얻는가
    카메라 좌표계 병진은 항상 (0, 0, Z) 라 옆으로 움직이지 않습니다. 그러나 타겟이
    매 프레임 무작위로 회전하므로, 두 뷰 사이의 상대 자세를 계산하면 타겟 기준
    으로는 카메라가 궤도를 돈 것과 같아집니다.

        R_ij = R_j · R_i^T
        t_ij = t_j - R_ij · t_i

    거리 5~6 m 에서 회전각 4 도면 유효 베이스라인이 약 0.45 m 생깁니다.
    이 (R_ij, t_ij) 를 cv2.stereoRectify 에 넣으면 평행 정렬된 쌍이 되어
    표준 StereoSGBM 을 그대로 쓸 수 있습니다.

    다만 조건이 까다롭습니다. 회전각이 크면 표면 대응이 끊기고, 병진이 광축 방향에
    치우치면(전방 이동) 에피폴이 화면 안으로 들어와 정합이 무너집니다. 그래서
    회전각이 작으면서 횡방향 성분이 지배적인 쌍만 골라야 합니다.
"""
...
# SGBM 정합 블록 크기의 기본값. 근거는 compute_disparity() 의 설명에 있습니다.
# 한 곳에서만 정하지 않으면 reconstruct() 의 기본값이 조용히 다른 값을 씁니다.
...
def baseline_geometry(pose_i, pose_j) -> dict:
    """두 뷰 사이 베이스라인의 크기와 방향 성분을 정리합니다."""
    ...
    return {
        ...
        "lateral": lateral,
        "forward": forward,
        # 이 비가 클수록 옆으로 나란한 배치에 가까워 정합이 안정적입니다.
...
def find_pairs(model, max_rotation_deg: float = 8.0,
    ...
    """스테레오에 쓸 만한 뷰 쌍을 고릅니다.

    선별 기준
        1. 두 뷰의 자세 차이가 작을 것 — 표면 대응이 살아 있어야 합니다
        2. 베이스라인의 횡방향 성분이 전방 성분보다 클 것 — 전방 이동은
           에피폴이 화면 안에 들어와 정합이 무너집니다
...
class RectifiedPair:
    ...
    정렬 창을 원본과 같은 크기로 두면 안 됩니다
        정렬은 두 카메라를 평행하게 만드는 회전이라, 정렬된 평면에서 원본 화면은
        기울어진 사각형이 됩니다. 그 사각형의 경계 상자는 원본보다 큽니다. 창을 원본
        크기 그대로 두면 OpenCV 가 잡아 주는 위치에서 잘려 나가고, 타겟이 그
        바깥에 걸리면 조용히 사라집니다. 후보 20 쌍 중 14 쌍에서 타겟이 창 테두리에
        닿아 있었고, 심한 쌍은 실루엣의 절반을 잃고 있었습니다.

        잘린 부분은 마스크에서도 함께 사라지므로 "커버리지가 낮습니다"로도 안 잡힙니다.
        채점 영역 자체가 줄어들 뿐입니다.

    fit_window=True (기본)
        원본 네 귀퉁이가 정렬 평면 어디에 떨어지는지 계산해 아무것도 잘리지 않는
        최소 창을 잡습니다. 초점거리는 그대로 두고 주점만 옮기므로 시차와
        베이스라인은 변하지 않습니다. 최적 쌍에서 채점 영역이 8,442 -> 9,485 화소로
        늘고 RMSE·중앙값·5cm·유효화소가 모두 함께 좋아집니다.

        newImageSize 로 창을 키우는 방법과는 다릅니다. 그쪽은 OpenCV 가 화각을
    ...
    def _fit_window(camera, R1, P1, P2, max_side):
        """원본이 잘리지 않는 최소 정렬 창과 그에 맞춘 투영 행렬을 만듭니다."""
        ...
        # 회전이 심한 쌍에서는 경계 상자가 수천 화소까지 커집니다. 정합 비용이
        # 넓이에 비례하므로 상한을 두고, 넘으면 전체를 균일하게 축소합니다.
        # 축소해도 잘리지는 않습니다.
    ...
    def __init__(self, camera, R_ij, t_ij, alpha: float = -1.0, size=None,
        ...
        # 베이스라인이 세로 방향이면 OpenCV 는 오프셋을 P2[1, 3] 에 싣습니다.
    ...
    def match_width(self) -> int:
        ...
        세로 길이가 됩니다. 시차 탐색 폭의 상한은 이 값으로 잡아야 합니다.
        정사각 영상에서는 둘이 같아 차이가 드러나지 않습니다.
        """
    ...
    def remap(self, left, right, mask=None):
        ...
        if not self.horizontal:
            # 세로 스테레오는 90도 돌려 가로로 만든 뒤 표준 매처를 씁니다.
...
def reference_depth(pair: "RectifiedPair", pose, points_body) -> np.ndarray:
    """동체 좌표계 점들을 정렬된 왼쪽 카메라로 투영해 기준 깊이 맵을 만듭니다.

    reconstruct() 와 짝입니다. 둘이 같은 좌표계를 돌려줘야 화소 단위로 비교할 수
    있는데, 좌표 규약의 양쪽이 서로 다른 파일에 흩어져 있으면 한쪽만 고치는
    사고가 납니다. 실제로 그랬습니다. reconstruct 가 세로 쌍에서 돌아간 채로 반환하던
    것을 고치면서 이 함수의 unrotate 를 안 걷어내, 기준 깊이가 90 도 어긋난
    채로 채점되고 있었습니다. 마스크 안에서 기준 깊이가 정의된 비율이 27.9% 까지
    떨어졌습니다. 그래서 같은 모듈로 옮기고 테스트를 붙였습니다.

    pair.camera 는 돌리기 전 정렬 카메라이므로 결과는 이미 정렬된 왼쪽 카메라
    좌표계입니다. 방향을 더 손대면 안 됩니다.

    Parameters
    ----------
    points_body : (N, 3) 동체 좌표계 점. 메시를 조밀하게 샘플링한 것을 넣습니다.
    """
...
def compute_disparity(left, right, num_disparities: int,
    ...
    """StereoSGBM 으로 시차 맵을 만듭니다. 매칭 실패는 NaN.

    block_size 기본값을 11 로 둔 근거

        SGBM 은 블록 안에서 시차가 일정하다고 가정합니다. 블록이 크면 대응을
        찾기 쉬워지지만 기울어진 면에서 깊이가 뭉개지고, 작으면 반대가 됩니다.
        어느 쪽이 유리한지는 **장면에 무늬가 얼마나 있는지**로 갈립니다.
        ...
        크레이터와 그림자로 무늬가 넉넉해 **작은 블록이 정확합니다.** 그래서 달
        실험은 호출부에서 5 를 씁니다. 여기 기본값 11 은 무늬가 적은 장면까지
        고려한 무난한 값이고, 실제로 쓸 때는 데이터에서 다시 골라야 합니다.

        **교훈은 값이 아니라 절차입니다.** 한 데이터에서 고른 하이퍼파라미터를
        다른 데이터에 검증 없이 옮기면 안 됩니다. 병목이 다르기 때문입니다.
        uniquenessRatio 는 1~10 사이에서 결과가 거의 바뀌지 않아 5 를 유지합니다.
...
def filter_disparity(disparity, max_speckle_size: int = 400,
    ...
    왜 필요한가
        ...
        작은 덩어리로 만들어냅니다. 화소 수는 적지만 시차가 조금만 틀려도 깊이는
        Z = f·B/d 를 따라 크게 튀므로 RMSE 를 지배해 버립니다.

    측정 효과 (달 지형 티코 크레이터 기준. 나머지 조건은 고정하고
    필터만 바꾼 값이며 run_3d_experiment.py 의 filter_ablation() 이 만듭니다)
    ...
    if max_speckle_size > 0:
        # filterSpeckles 는 16 배 고정소수점 int16 을 요구합니다.
    ...
    if median_kernel and median_kernel >= 3:
        ...
        # 0 이 되어 중앙값이 0 쪽으로 끌려갑니다. 시차 0 은 무한원점이므로 깊이가
        # 밀려납니다. 실측하면 경계 화소(3x3 안에 무효가 있는 화소)만 평균
        # -10.9 px 편향됐고 내부 화소는 -0.019 px 로 영향이 없었습니다. 즉 이 편향은
        # 전적으로 0 채움에서 옵니다. 유효 이웃이 창의 과반일 때만 median 을
        # 적용하고, 그렇지 않으면 원래 값을 둡니다.
...
def local_contrast(image, ksize: int = 7) -> np.ndarray:
    """화소 주변의 밝기 표준편차. 정합할 단서가 얼마나 있는지를 나타냅니다.

    블록 정합은 창 안의 밝기 패턴을 맞춥니다. 그 패턴이 평평하면 어디에 갖습니다
    대도 비용이 비슷해서, 매처는 탐색 구간 안 어딘가를 고르기는 하지만 그것이
    옳다는 보장이 없습니다.

    표준편차는 적분 영상으로 한 번에 구합니다.  Var = E[x^2] - E[x]^2
    """
...
def texture_mask(image, min_contrast: float, ksize: int = 7) -> np.ndarray:
    ...
    왜 필요한가 — 실측 (달 지형 티코 크레이터, 시차 1 px = 450 m)
        ...
        하한 2 는 화소를 2.6 %p 만 내주고 **크게 틀린 화소를 3분의 1 줄입니다.**
        더 올리면 덮는 범위만 빠르게 잃습니다. 크게 틀린 화소들은 실제로 어두운
        곳에 몰려 있었습니다(평균 밝기 62 대 전체 114) - 크레이터 안쪽 그늘입니다.

        다만 이것으로 "대응이 없습니다" 를 가려낼 수는 없습니다. 좌우에 같은 영상을
        넣어도 화소의 21% 가 값을 내는데, 관문을 걸어도 20% 로 거의 줄지
        않습니다. 매처는 탐색 구간 안에서 반드시 무언가를 고르기 때문입니다.
        신뢰도를 함께 내보내는 것이 제대로 된 해법이고, README 9절에 적었습니다.
    """
    if min_contrast < 0:
...
def disparity_to_depth(disparity, focal: float, baseline: float) -> np.ndarray:
    ...
    이 한 줄이 과제가 요구한 '깊이 맵 생성'의 본체입니다. 밝기와 달리 이 값은
    카메라 기하에서 유도되므로 미터 단위 물리량입니다.
    """
    if focal <= 0:
...
def reconstruct(pair: "RectifiedPair", left, right, mask=None,
    ...
    """정렬된 쌍에서 깊이 맵과 포인트 클라우드를 만듭니다.

    Parameters
    ----------
    depth_range : (lo, hi) 밖의 깊이를 버립니다. 타겟 크기를 알면 물리적으로
        불가능한 값을 걸러낼 수 있습니다. 호출부(run_3d_experiment.py)는
        [d0 - R, d0 + R] 을 쓰는데, 창의 **폭** 2R 은 타겟 치수에서 오지만
        창의 **중심** d0 는 데이터셋의 정답 거리입니다. 화소 단위 정답 깊이를
        쓰는 것은 아니지만 정답 자세에 의존하는 것은 맞습니다. 실제 상대항법이면
        추정 거리를 넣어야 합니다. README 7절 한계에 함께 적었습니다.
    postfilter : filter_disparity() 로 이상치를 제거할지 여부.
    block_size : SGBM 정합 블록 크기. 기본 3 은 합성 장면에서 고른 값이라
        실데이터에 그대로 맞는다는 보장이 없습니다. 호출부에서 실측으로
        비교할 수 있게 열어 둔다 (README 8절 개선점).

    Returns
    -------
    dict(left, right, mask, disparity, depth, points, valid, num_disparities)
        화소 맵은 모두 **정렬된 왼쪽 카메라 좌표계**로 돌려줍니다. 세로 스테레오
        에서 매칭을 위해 돌렸던 것은 반환 전에 되돌리므로, 호출부는 가로/세로를
        구분할 필요가 없습니다. points 는 그 좌표계의 (N, 3) 입니다.
    """
    ...
    # 세로 스테레오는 remap() 에서 90도 돌려 매칭했습니다. 반환하기 전에 되돌려
    # 놓아야 합니다. 그러지 않으면 호출부가 돌아간 깊이 맵을 똑바로 선 기준
    # 깊이와 비교하게 되고, pair.camera 는 돌리기 전 정렬 카메라이므로
    # unproject() 도 틀린 광선을 씁니다. 영상이 정사각이면 shape 이 같아서
    # 예외 없이 조용히 틀립니다.
...
# 정답 없이 쓰는 신뢰도
#
# 지금까지는 어느 화소가 맞았는지를 정답 고도와 비교해야만 알 수 있었습니다.
# 실제 운용에는 정답이 없으므로, 두 영상만으로 계산되는 단서가 필요합니다.
# 아래 셋은 모두 정답을 쓰지 않습니다.
# ---------------------------------------------------------------------------
...
def compute_disparity_both(left, right, num_disparities: int,
    ...
    """왼쪽 기준 시차와 오른쪽 기준 시차를 함께 구합니다.

    오른쪽 기준 시차는 두 영상을 좌우로 뒤집어 역할을 맞바꾼 뒤 정합하고
    결과를 다시 뒤집으면 얻어집니다. 뒤집으면 시차의 방향도 뒤집히므로 같은
    부호 규약으로 돌아옵니다. StereoSGBM 을 한 번 더 돌리는 것 말고는 새로
    필요한 것이 없습니다.
    """
...
def left_right_consistency(disparity_left, disparity_right,
    ...
    왜 이것이 필요한가
        한쪽 영상에서만 보이는 곳(가림)에서 매처는 반드시 무언가를 고릅니다.
        고를 대상이 없는데도 값을 내므로, 그 값이 틀렸다는 사실을 매처 자신은
        알려 주지 않습니다. 왼쪽 기준으로 x 가 x-d 에 대응한다면, 오른쪽 기준
        으로도 x-d 가 x 를 가리켜야 합니다. 가림에서는 이 왕복이 깨집니다.

        크레이터 안쪽 그늘처럼 한쪽에만 보이는 곳이 정확히 여기서 걸립니다.
        정답 고도를 전혀 쓰지 않는다는 점이 핵심입니다.
...
def photometric_residual(left, right, disparity, ksize: int = 7) -> np.ndarray:
    ...
    머뭅니다. 틀린 화소는 다른 지점을 겹치므로 잔차가 큽니다. 정답이 아니라
    입력 영상 두 장만 씁니다.
    ...
    # 상자 필터는 NaN 을 퍼뜨립니다. 유효한 값만 더하고 그 개수로 나눕니다.
...
def sparsification(error, confidence, steps: int = 20):
    ...
    무엇을 하는가
        ...
        중앙값을 기록합니다. 신뢰도가 쓸모 있다면 버릴수록 오차가 빠르게
        떨어집니다. 같은 일을 **실제 오차 순서로** 하면 이론상 가장 빠르게
        떨어지는 곡선(오라클)이 나옵니다. 두 곡선 사이의 넓이가 AUSE 다.

        0 에 가까울수록 그 신뢰도가 오차를 잘 예측한다는 뜻입니다. 신뢰도와
        오차가 무관하면 곡선이 평평해져 넓이가 커집니다.

    여기서 정답을 쓰는 것은 **채점할 때뿐**입니다. 신뢰도 자체는 정답 없이
    계산된 값이어야 하며, 그것이 이 함수를 쓰는 이유입니다.
    ...
    def curve(order):
        # 평균 절대오차로 그립니다. 중앙값으로 그리면 크게 틀린 소수를 버리는
        # 효과가 곡선에 거의 나타나지 않아, 신뢰도가 하는 일을 과소평가합니다.
        out = []
        for f in fractions:
    ...
    # numpy 2 에서 trapz 가 사라졌습니다. 사다리꼴 적분을 직접 씁니다.
...
def rank_correlation(a, b) -> float:
    """스피어만 순위 상관. 두 값의 순서가 얼마나 같이 가는지만 봅니다.

    신뢰도와 오차의 관계는 직선이 아니므로 피어슨 상관은 뜻이 흐립니다.
    순위로 바꾸면 "신뢰도가 낮은 화소가 실제로도 더 틀리는가" 라는 질문에
    바로 답합니다. 1 에 가까울수록 그렇다는 뜻입니다.
    """
...
#
# 상수로 박아 둔 값은 그 지형에서 훑어 고른 값입니다. 지형이 바뀌면 다시
# 골라야 하는데, 그러면 자동으로 도는 파이프라인이 아닙니다. 영상 자체에서
# 유도하면 따라옵니다.
# ---------------------------------------------------------------------------
...
def estimate_noise_sigma(image) -> float:
    ...
    모두 지웁니다. 남는 것은 잡음뿐이므로, 그 응답의 평균 절대값에서 잡음
    크기를 되돌릴 수 있습니다. 지형이 무엇이든 영상 한 장이면 됩니다.
...
def autocorrelation_length(image, max_lag: int = 32) -> float:
    ...
    무늬가 잘면 자기상관이 금방 떨어지고, 밋밋하면 오래 갑니다. 정합 블록은
    "무늬 하나가 들어갈 만큼" 이어야 하므로 이 길이가 블록 크기의 기준이
    됩니다. 창 안에 무늬가 하나도 없으면 어디에 맞춰도 비슷해 보이고, 너무
    크면 경사면에서 깊이가 뭉개집니다.
    """
    img = np.asarray(image, dtype=np.float64)
    if img.ndim != 2:
...
def contrast_floor(image, k: float = 2.0) -> float:
    """대비 하한을 잡음 크기의 배수로 정합니다.

    지금까지 쓰던 "회색조 2단계" 는 이 영상에서 훑어 고른 절대값이라,
    밝기 분포가 다른 영상에 그대로 쓰면 뜻이 달라집니다. 잡음의 k 배로 두면
    "잡음보다 이만큼은 뚜렷한 무늬" 라는 같은 뜻이 어디서나 유지됩니다.
    """
    if k <= 0:
...
def _windowed_zncc(a, b, valid, ksize):
    ...
    차이에 흔들리지 않는다 — 시점에 따라 밝기가 달라지는 달 표면에서 중요합니다."""
...
def matching_margin(left, right, disparity, ksize: int = 9):
    """이긴 시차가 이웃보다 얼마나 뚜렷하게 이겼는지 잽니다.

    왜 이것이 가장 강한 단서인가
        매처는 탐색 구간에서 비용이 가장 낮은 시차를 고릅니다. 그 최소가
        **뾰족하면** 다른 후보와 확실히 구별된 것이고, **평평하면** 아무
        데나 골라도 비슷했다는 뜻입니다. 정합기 안에서 실제로 일어난 일에
        가장 가까운 단서이며, 정답을 쓰지 않습니다.

        OpenCV 의 StereoSGBM 은 비용 부피를 밖으로 내주지 않습니다. 그래서
        이긴 시차와 그 양옆에서 상관을 **다시 재서** 봉우리 모양을 복원합니다.
...
def rank_fuse(*signals):
    ...
    단위도 분포도 다른 신호를 그냥 더할 수는 없습니다. 각각을 순위(0~1)로 바꾸면
    같은 자 위에 놓입니다. 하나가 놓치는 것을 다른 하나가 잡을 때 합이 각각보다
    나아진다 — 실제로 그런지는 재 봐야 압니다.
    """
    if not signals:
...
def blunder_auc(error, confidence, threshold: float) -> float:
    ...
    왜 채점 대상을 바꾸는가
        오차의 대부분은 부화소 잡음이고, 그것은 원리적으로 예측할 수 없습니다.
        실제로 필요한 판단은 "이 값을 버릴까 말까" 이므로, **크게 틀린 화소를
        골라내는가** 를 물어야 합니다. 희소화 곡선이 잘 안 나오는 것과 이 질문에
        잘 답하는 것은 동시에 성립할 수 있습니다.

        0.5 가 무작위, 1.0 이 완벽입니다. 순위 통계로 계산하므로 문턱값을
        훑을 필요가 없습니다.
    """
    err = np.asarray(error, dtype=np.float64).ravel()
    conf = np.asarray(confidence, dtype=np.float64).ravel()
    ...
    # 신뢰도가 낮을수록 크게 틀렸다고 예측하는 것이므로 부호를 뒤집습니다.
    pred = -conf
