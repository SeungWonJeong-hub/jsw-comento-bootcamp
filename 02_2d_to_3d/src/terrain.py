"""달 지형(DTM)에서 스테레오 영상 쌍을 만듭니다.

왜 이 모듈이 필요한가
    궤도에서 찍은 원본 영상으로 곧장 스테레오를 돌리려면 카메라 자세를 담은
    커널을 읽고 지도 투영을 맞추는 전용 도구 체계가 필요합니다. 그 준비만으로
    며칠이 갑니다.

    대신 **이미 측정된 고도 모델(DTM)** 에서 출발합니다. 고도를 3D 점으로 펴고,
    자세를 아는 카메라 두 대로 다시 찍습니다. 그러면
      - 정답 고도가 처음부터 손에 있고 (레이저 고도계로 잰 값)
      - 카메라 자세도 우리가 정한 값이라 정확하며
      - 복원 결과를 원래 고도와 바로 견줄 수 있습니다.

    착륙 위험 탐지 연구에서 실제로 쓰는 방식입니다. 다만 **기하는 실제 달이지만
    밝기는 렌더링**이라는 점은 분명히 해야 합니다. 그림자와 표면 반사 특성은
    실제 영상과 다릅니다.

좌표 규약
    지형 좌표계는 X 동쪽, Y 북쪽, Z 위(고도)다. 카메라는 이 좌표계 안의
    위치와 자세로 놓습니다. 카메라 좌표계는 저장소의 다른 모듈과 같습니다
...
#: g 는 도 단위이며, L(0)=1 에서 시작해 위상각이 커질수록 0 쪽으로 갑니다.
#: NASA Ames Stereo Pipeline 과 ISIS 가 달에 쓰는 값과 같습니다.
...
def load_dtm(path: str, crop: int = None):
    ...
    때문입니다. 카메라 초점거리도 "고도 / 화소 크기" 로 정합니다.
    """
    import rasterio
    ...
    # 빈 값은 주변 평균으로 메웁니다. 스테레오는 구멍을 만나면 그 자리에서
    # 엉뚱한 대응을 만들어 내므로, 없는 곳을 남겨 두면 안 됩니다.
...
def synthetic_dtm(size: int = 512, gsd: float = 5.0, relief: float = 300.0,
    ...
    테스트도 데이터 없이 통과해야 합니다. 큰 굴곡 위에 크레이터를 파 넣습니다.

    크레이터를 넣는 이유는 무늬 때문입니다. 매끈한 경사면만 있으면 정합할
    단서가 없어 시차를 못 찾습니다. 실제 달 표면이 정합이 잘 되는 것도
    크레이터와 바위가 만드는 그림자 덕분입니다.
    """
    if size < 32:
        raise ValueError(f"size 는 32 이상이어야 합니다: {size}")
    rng = np.random.default_rng(seed)

    # 저주파 굴곡: 무작위 잡음을 여러 배율로 겹칩니다.
...
def surface_points(elev: np.ndarray, gsd: float) -> np.ndarray:
    """고도 격자를 (N, 3) 지형 좌표 점으로 폅니다. X 동, Y 북, Z 고도."""
    if gsd <= 0:
        raise ValueError(f"화소 크기는 양수여야 합니다: {gsd}")
    h, w = np.asarray(elev).shape
    y, x = np.mgrid[0:h, 0:w]
    # 격자의 가운데를 원점으로 둡니다. 카메라를 원점 위에 놓기 편합니다.
    X = (x - (w - 1) / 2.0) * gsd
    Y = ((h - 1) / 2.0 - y) * gsd
...
def shade(elev: np.ndarray, gsd: float, sun_elevation_deg: float = 25.0,
    ...
    무늬가 살아나기 때문입니다. 실제 달 영상도 태양이 낮을 때 찍은 것이
    지형 판독에 쓰입니다. 태양이 머리 위면 표면이 밋밋해져 정합이 어려워집니다.

    viewer 를 주면 달 광도함수 (Lunar-Lambert) 를 씁니다
        램버트 반사는 밝기가 태양 방향에만 의존합니다. 그러면 같은 지면
        조각이 두 사진에서 **정확히 같은 밝기**로 찍힙니다. 수렴각을 아무리
        벌려도 밝기는 변하지 않으므로, 정합기 입장에서는 실제보다 훨씬 쉬운
        문제가 됩니다.

        실제 달 표면은 후방산란이 강해서 보는 방향에 따라 밝기가 달라집니다.
        그래서 수렴 촬영의 두 사진은 같은 조각을 다른 밝기로 담습니다. 이
        ...
        L=0 이면 램버트, L=1 이면 Lommel-Seeliger 가 되는 보간식입니다.
        위상각이 커질수록 L 이 작아지며 램버트 쪽으로 갑니다.
    ...
    denom = mu0 + mu
    # 입사와 방출이 모두 스치는 각이면 두 항이 함께 0 이라 값이 없습니다.
...
def sensor_image(intensity, snr: float = 100.0, blur_px: float = 0.7,
    ...
    """렌더링한 밝기를 실제 카메라가 찍은 것처럼 망가뜨립니다.

    왜 필요한가
        렌더링 영상은 잡음이 0 이고 초점이 완벽합니다. 그런 영상에서 정합이
        잘 되는 것은 당연하며, 그 성적을 실제 영상에 옮겨 말할 수 없습니다.
        실제 궤도 카메라에는 광학 흐림(PSF)이 있고, 빛은 광자 단위로 오므로
        밝기에 따라 커지는 산탄 잡음이 있으며, 읽어 낼 때 잡음이 더해지고,
        마지막에 정해진 비트 수로 잘립니다. 넷을 모두 넣습니다.

    Parameters
    ----------
    snr : 최대 밝기에서의 신호 대 잡음비. 광자 수 N 의 잡음이 sqrt(N) 이므로
          가득 찬 우물의 광자 수를 snr^2 으로 잡는 것과 같습니다. LROC NAC 이
          대략 100 수준입니다.
...
def look_at(position, target, up_hint=(0.0, 1.0, 0.0)) -> Pose:
    """지형 좌표계의 한 점을 바라보는 카메라 자세를 만듭니다.

    카메라 좌표계는 z 가 시선, x 가 영상 오른쪽, y 가 영상 아래쪽입니다.

    up_hint 를 **수평 방향(기본 북쪽)** 으로 두는 이유
        아래를 내려다보는 카메라는 시선이 거의 -Z 라, 흔히 쓰는 up=(0,0,1)
        과 거의 나란해집니다. 그러면 외적이 0 에 가까워져 영상의 오른쪽이
        시선의 미세한 수평 성분으로 정해지고, 카메라 위치가 조금만 달라져도
        방향이 홱 뒤집힙니다. 실제로 수렴 촬영 두 대의 상대 회전이 20도가
        아니라 180도로 나왔습니다. 수평 기준을 주면 두 대가 같은 쪽을 오른쪽으로
        삼습니다.
    """
...
def _scatter(points_cam, intensity, camera, splat):
    """한 번의 z-buffer 통과. 가장 가까운 점의 깊이와 그 점의 밝기를 남깁니다."""
    z = points_cam[:, 2]
    ok = z > 1e-9
    if not np.any(ok):
...
def render(points: np.ndarray, intensity: np.ndarray, camera, pose: Pose,
           splat: int = 1):
    """지형 점들을 카메라에 투영해 영상과 깊이 맵을 함께 만듭니다.

    splat 을 **빈 화소를 메우는 데만** 쓰는 이유
        점을 주변 화소까지 번지게 하면서 깊이도 함께 쓰면, 깊이 버퍼에 3x3
        최소값 필터를 건 것과 같아집니다. 표면이 카메라 쪽으로 당겨지고, 그
        당김은 표면 기울기 방향을 따릅니다. 수렴 촬영의 두 카메라는 서로 반대로
        기울어 있으므로 당김 방향도 반대가 되어, **시차가 한쪽으로 치우칩니다.**

        실측하면 티코 지형에서 splat=1 이 시차를 +2.6 px 밀어 깊이가 1.2 km
        치우쳤습니다. splat=0 으로 두면 +0.6 px 로 줄지만 이번에는 구멍이 남습니다.

        그래서 두 번 훑습니다. 먼저 번지지 않고(splat=0) 정직한 깊이를 얻고,
        그때 비어 있는 화소만 번진 통과로 메웁니다. 채워 넣은 자리는 원래
        정보가 없던 곳이라 치우침을 만들지 않습니다.
...
def render_heightfield(elev, gsd, intensity, camera, pose: Pose, iters: int = 12):
    ...
    왜 흩뿌리기(scatter)를 버렸는가
        점을 화소에 던지는 방식은 위치를 반올림합니다. 그 반올림 오차가 표면
        기울기 방향으로 쏠리는데, 수렴 촬영의 두 카메라는 반대로 기울어
        있으므로 좌우가 서로 다른 쪽으로 쏠립니다. 결과가 시차 치우침입니다.
        티코에서 splat 을 끄고도 +0.6 px 가 남았습니다.

        광선을 쏘면 화소 중심이 정확히 정의되고 지형은 이중선형으로 뽑으므로
        반올림이 없습니다. 구멍도 생기지 않습니다.

    푸는 방법
        고도장 z(X, Y) 와 광선 P = C + t·d 의 교점은 닫힌 형태가 없습니다.
        z_s 를 현재 추정 고도로 두고 t = (z_s - C_z)/d_z 로 t 를 갱신하는
        고정점 반복으로 풉니다. 기복이 촬영 거리보다 훨씬 작으면 몇 번에
        수렴합니다.

    수렴하지 않는 광선
        지형 격자 밖으로 나간 광선은 고도 표본이 가장자리 값으로 고정되어
        값이 진동합니다. 실측하면 그런 광선이 약 10% 있었고, 수렴하지 않은 채
        깊이를 내놓아 최악 300 m 까지 틀렸습니다. 마지막 갱신 폭이 화소 크기의
        1/100 을 넘으면 값을 내지 않는다 - "모릅니다" 를 틀린 값으로 채우는
        것보다 비워 두는 편이 낫습니다.

    한계
        가림(occlusion)을 다루지 않습니다. 광선이 앞쪽 봉우리를 지나쳐 뒤쪽
        지면에 닿아도 그대로 받습니다. 시선이 지면에 거의 수직이고 경사가
        완만하면 문제가 없지만, 수렴각을 크게 키우면 확인이 필요합니다.
    """
    ...
    # 만납니다)가 깨집니다. 값이 거의 안 나오는 결과가 조용히 돌아오는 대신
    # 여기서 멈춥니다. 실제로 기복 4 km 지형을 고도 2.5 km 에서 찍는 설정이
    # 유효화소 3% 를 내놓고도 아무 말이 없었습니다.
    ...
    for _ in range(iters):
        ...
        z_guess = z_new
    # 반복이 끝난 뒤 한 번 더 갱신합니다. 그러지 않으면 t 가 마지막 고도 표본보다
    # 한 걸음 뒤처져, 수렴했는데도 잔차가 남습니다.
    tt = (z_guess - centre[2]) / d[:, 2]
...
def load_image(path: str, crop: int = None):
    """실제로 찍은 영상을 읽는다 (고도 모델과 같은 격자여야 합니다)."""
    import rasterio
...
def estimate_illumination(image, elev, gsd, step_deg: float = 5.0):
    """찍힌 영상이 어느 방향에서 비춰진 것인지 지형으로부터 되찾습니다.

    왜 필요한가
        모자이크 영상에는 촬영 당시의 음영이 이미 들어 있습니다. 그것을 그대로
        알베도로 쓰면 **음영을 두 번 입히게 된다** — 우리가 다시 조명을 걸기
        때문입니다. 빼내려면 원래 조명 방향을 알아야 하는데, 모자이크에는
        그 값이 붙어 있지 않습니다.

        지형의 법선은 알고 있으므로, 여러 태양 방향을 넣어 보고 영상과 가장
        잘 맞는 방향을 고르면 됩니다. 음영이 지형에서 온 것이라면 반드시 어떤
        방향에서 상관이 뚜렷하게 높아집니다.

    Returns
    -------
    (방위각, 고도, 상관계수) — 상관이 낮으면 음영보다 무늬가 지배적이라는 뜻입니다.
    """
    n = surface_normals(elev, gsd)
...
def derive_albedo(image, elev, gsd, mean_albedo: float = 0.12,
                  floor: float = 0.25):
    """찍힌 영상에서 음영을 빼내고 표면 무늬만 남깁니다.

    영상 = 알베도 x 음영 이므로, 추정한 조명으로 만든 음영으로 나누면 알베도가
    남습니다. 스치는 각에서는 음영이 0 에 가까워 나눗셈이 폭발하므로 바닥을 둡니다.

    남는 것은 티코의 광조(ray) 무늬, 어둡고 밝은 물질, 고도 모델에는 없는
    잔크레이터입니다. **고도 모델이 매끈해서 잔무늬가 적다** 는 한계를 이것이 메웁니다.
