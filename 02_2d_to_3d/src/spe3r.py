...
데이터 구조에서 확인한 사실 (aqua 기준, 직접 측정)
    ...
    => 카메라 좌표계만 보면 스테레오 베이스라인이 없다. 그러나 필요한 것은
       카메라의 절대 운동이 아니라 카메라-타겟의 상대 운동이다. 타겟이 돌므로
       상대 자세 (R_ij, t_ij) 에서 유효 베이스라인이 드러난다 (src/stereo.py).
       조건은 까다롭다. 회전각이 작으면서 횡방향 성분이 지배적인 쌍만 쓸 수
       있어 1,000 뷰에서 후보가 20 쌍뿐이다.
"""
