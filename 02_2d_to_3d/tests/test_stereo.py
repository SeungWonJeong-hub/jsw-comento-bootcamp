...
각 단계를 손으로 풀 수 있는 조건에서 수치까지 검증합니다.
"""
...
256x256 에서 베이스라인 0.4 m 를 쓰면 시차가 약 102 px 인데, SGBM 은
    왼쪽 numDisparities 폭을 통째로 무효 처리합니다. 물체 대부분이 그 무효
    영역에 들어가므로, 초점거리는 같게 두고 화면만 넓혀 여유를 확보합니다.
    """
...
def test_baseline_scaling_invariance():
    """베이스라인과 시차를 함께 2배 하면 깊이는 그대로입니다."""
    f = 1000.0
    d = np.array([[40.0]])
...
def test_nonpositive_disparity_becomes_nan():
    """시차 0 은 무한원점을 뜻합니다. 0 나눗셈이 새어 나가면 안 됩니다."""
...
def test_relative_pose_maps_view_i_to_view_j():
    """p_j = R_ij p_i + t_ij 가 실제로 성립해야 합니다."""
...
def test_target_rotation_creates_lateral_baseline():
    """카메라 병진이 (0, 0, Z) 로 같아도 타겟이 돌면 횡방향 베이스라인이 생깁니다.

    이것이 한 대의 카메라로도 스테레오가 성립하는 이유입니다. 거리 d 에서 각 t 만큼
    돌면 유효 베이스라인은 2·d·sin(t/2) 입니다.
    """
    dist = 5.5
...
def test_rectified_pair_recovers_baseline(cam):
    """순수 횡방향 이동이면 정렬 후 베이스라인이 원래 크기와 같아야 합니다."""
    B = 0.4
...
def test_fit_window_keeps_geometry_and_contains_the_source(cam):
    """정렬 창을 원본이 잘리지 않는 크기로 잡되, 시차 기하는 그대로여야 합니다.

    창을 넓히는 것은 주점을 옮기는 것뿐이므로 초점거리와 베이스라인이 변하면
    안 됩니다. 변하면 Z = f·B/d 가 통째로 어긋납니다.
    """
    B = 0.4
    ...
    # 원본 네 귀퉁이가 전부 창 안에 들어와야 합니다.
    rng = np.random.default_rng(2)
    img = rng.integers(1, 255, size=cam.shape, dtype=np.uint8)
    L, _, _ = fitted.remap(img, img)
    assert (L > 0).sum() >= (img > 0).sum() * 0.99, "정렬 창이 원본을 자릅니다"


def test_fit_window_respects_the_size_cap(cam):
    """회전이 심하면 경계 상자가 수천 화소가 됩니다. 상한을 두고 균일 축소합니다.

    실제 데이터에서는 창 최대변이 423 이라 이 분기가 한 번도 실행되지 않습니다.
    안 타는 코드는 틀려도 모르므로 여기서 강제로 태웁니다. 축소는 균일해야
    하므로 Z = f·B/d 의 f·B 비율이 보존돼야 합니다.
    """
    B = 0.4
    ...
    assert max(small.size) <= 64, f"상한을 넘었습니다: {small.size}"
    assert max(big.size) > 64, "이 테스트는 상한이 실제로 걸리는 조건을 전제로 합니다"

    # 균일 축소이므로 초점거리는 줄고 베이스라인은 그대로입니다.
    assert small.focal < big.focal
    assert small.baseline == pytest.approx(big.baseline, rel=1e-9)
    # 같은 거리에서 예상 시차는 초점거리에 비례해 함께 줄어야 합니다.
...
#
# 후보 20 쌍 중 절반이 세로입니다. 이 경로가 조용히 틀리면 후보의 절반을 통째로
# 잃는데, 영상이 정사각(256x256)이면 shape 이 같아 예외가 나지 않습니다.
# --------------------------------------------------------------------------

def test_vertical_baseline_is_detected(cam):
    """세로 이동이면 horizontal 이 False 여야 하고 베이스라인은 그대로입니다."""
    B = 0.4
...
def test_vertical_remap_rotates_and_unrotate_restores(wide_cam):
    """remap 은 90도 돌려 가로로 만들고, unrotate 는 정확히 되돌려야 합니다."""
