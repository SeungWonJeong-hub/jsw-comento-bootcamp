...
렌더링을 실제 촬영에 가깝게 만드는 도구다. 그래서 "그럴듯한 값이 나온다"
로는 검증이 안 된다. 손으로 답을 낼 수 있는 조건을 만들어 숫자까지 맞춘다.

  해석해      평평한 면에 달 광도함수를 직접 계산해 렌더링 값과 대조한다
  통계        잡음을 넣은 만큼 추정되는가, 광자 잡음이 밝기를 따라 커지는가
  불변식      같은 시차끼리는 좌우 일관성을 통과하고, 어긋난 곳은 걸린다
  대조군      신뢰도가 무작위보다 나은지를 같은 자로 잰다
...
def test_lambert_shading_is_identical_from_every_viewpoint():
    """램버트 반사는 보는 방향과 무관하다 — 그래서 실제보다 쉬운 문제다.

    이 테스트는 통과하는 것이 목적이 아니라, **왜 램버트로는 안 되는지**를
    코드로 못 박아 두는 것이 목적이다. 두 사진의 같은 지면이 똑같은 밝기로
    찍히면 정합기는 실제로 겪지 않을 쉬운 문제를 푸는 셈이 된다.
    """
...
def test_lunar_lambert_differs_between_the_two_stereo_viewpoints():
    ...
    assert not np.allclose(a, b), "두 시점의 밝기가 같으면 시선이 안 들어간 것이다"

    # 램버트와도 달라야 한다. 같다면 mu 가 식에서 빠진 것이다.
    assert not np.allclose(a, terrain.shade(elev, gsd))


def test_lunar_lambert_matches_the_formula_computed_by_hand():
    """평평한 면에서는 각도가 전부 손으로 나오므로 값까지 대조할 수 있다.

    수평면의 법선은 (0, 0, 1) 이다. 카메라를 정확히 머리 위에 두면 방출각이
    0 이라 mu = 1 이고, mu0 은 태양 고도의 sin 이다. 위상각은 태양과 천정이
    이루는 각이므로 90도 - 태양고도다. 그러면 다음 값이 나와야 한다.

        I = A [ 2 L mu0 / (mu0 + 1) + (1 - L) mu0 ] + ambient
    """
    gsd, sun_el, albedo, ambient = 10.0, 40.0, 0.12, 0.06
    # 격자 한가운데 화소가 (0, 0) 이 되도록 홀수 크기를 쓴다.
    elev = np.zeros((31, 31))
...
def test_camera_center_matches_the_stereo_geometry():
    ...
    # 수렴 촬영이므로 두 대가 원점을 기준으로 좌우 대칭이다.
...
def test_photon_noise_grows_with_brightness():
    """광자 잡음은 밝기의 제곱근에 비례한다 — 어두운 곳이 상대적으로 더 나쁘다.

    이것이 크레이터 그늘에서 정합이 어려운 물리적 이유다. 잡음을 밝기와
    ...
    assert sb > sd, "밝은 곳의 잡음이 더 커야 광자 잡음이다"
    # 세기의 제곱근 비만큼. 읽기 잡음이 섞여 있으므로 느슨하게 본다.
    assert sb / sd == pytest.approx(np.sqrt(240 / 30), rel=0.5)


def test_sensor_image_is_reproducible_and_quantised():
    """같은 씨앗이면 같은 영상, 비트 수를 줄이면 값의 가짓수가 준다."""
...
def test_noise_estimate_recovers_the_noise_that_was_added():
    """넣은 잡음만큼 추정되어야 대비 하한을 잡음의 배수로 둘 수 있다."""
...
def test_contrast_floor_follows_the_noise_level():
    """대비 하한은 절대값이 아니라 잡음의 배수여야 영상이 바뀌어도 뜻이 같다."""
...
def test_autocorrelation_is_longer_for_smoother_texture():
    """무늬가 완만할수록 자기상관이 오래 간다 — 블록이 커야 한다는 뜻이다."""
    import cv2
...
def test_left_right_consistency_passes_matching_disparities():
    """같은 시차끼리는 통과한다. 단, 왼쪽 가장자리는 짝이 영상 밖이라 걸린다."""
    d = np.full((8, 20), 4.0)
    ok = stereo.left_right_consistency(d, d)
    assert ok[:, 4:].all(), "짝이 영상 안에 있으면 통과해야 한다"
    assert not ok[:, :4].any(), "x - d 가 음수면 짝이 없으므로 걸러야 한다"


def test_left_right_consistency_catches_a_mismatched_region():
    """한쪽만 다른 값을 가리키면 그 왕복은 깨진다 — 가림이 잡히는 원리다."""
    d_left = np.full((8, 20), 4.0)
    d_right = d_left.copy()
    d_right[:, 8:12] = 11.0        # 이 구간은 왼쪽을 도로 가리키지 않는다

    ok = stereo.left_right_consistency(d_left, d_right)
    # 왼쪽 x 는 오른쪽 x-4 를 본다. 오른쪽 8~11 이 어긋났으니 왼쪽 12~15 가 걸린다.
...
def test_sparsification_separates_a_perfect_signal_from_a_random_one():
    """희소화 곡선이 실제로 신뢰도의 좋고 나쁨을 가려내는지 본다.

    오차를 그대로 신뢰도로 쓰면 오라클과 같아지므로 AUSE 가 0 이어야 한다.
    무작위 신뢰도는 곡선이 평평해 넓이가 커진다. 이 두 극단을 구분하지
    못하면 이 자로 잰 어떤 값도 뜻이 없다.
    """
...
def test_rank_correlation_reads_monotone_relations():
    """순위 상관은 직선이 아니어도 같은 방향이면 1 이 나와야 한다."""
...
def test_compute_disparity_both_recovers_a_known_shift():
    """좌우 두 기준의 시차가 모두 실제 이동량을 되찾아야 한다."""
