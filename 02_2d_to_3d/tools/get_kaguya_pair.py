...
무엇을 받는가
    ...
    **스테레오 전용 카메라**다. 같은 궤도에서 같은 지역을 두 각도로 찍으므로,
    이 실험이 가정한 수렴 촬영과 기하가 같습니다. 수렴각은 약 30돕니다.

    USGS 가 그 관측을 지도 투영해 Cloud Optimized GeoTIFF 로 공개해 두었습니다.
    두 장이 **같은 좌표계의 같은 격자**에 놓이므로 카메라 자세 커널 없이도
    시차를 잴 수 있다 — 지형의 높낮이가 두 장 사이의 어긋남으로 남기 때문입니다.

    구면에 투영한 것이라 고도에 비례하는 어긋남이 그대로 남습니다.

        어긋남 = 고도 x (tan(e1) + tan(e2))

    e 는 각 카메라의 방출각입니다. 공칭 ±15도이면 계수가 0.536 이므로, 5 m 화소
    한 칸이 고도 약 9 m 에 해당합니다.
...
GSD = 5.0          # 공통 격자의 화소 크기 [m]. 원본이 4.79~4.96 이라 그 사이로 잡습니다.
...
def fetch(name, transform, width, height, crs, out_path):
    ...
    # 원본은 int16 이지만 실제 값은 12비트 근처입니다. 0~255 로 펴서 저장합니다.
    valid = dst > 0
    if not valid.any():
...
def main() -> int:
    ...
    if half * 2 > min(right - left, top - bottom):
        ...
        size = int(2 * half / GSD)
        print(f"  겹치는 범위가 좁아 {size} 화소로 줄입니다")
    else:
        size = args.size
