...
무엇을 받는가
    1971년 7월 31일, 아폴로 15호 사령선이 24초 간격으로 필름에 찍은 두 장입니다.
    매핑 카메라는 궤도를 돌며 78% 씩 겹치게 연속 촬영했으므로, 이웃한 두 장이
    그대로 스테레오 쌍이 됩니다.

    ASU 아폴로 영상 아카이브가 원본 필름 스캔을 공개하고 있습니다. 원본은 한 장에
    500 MB 가 넘으므로 4048 화소짜리 중간 판본을 받습니다. 그것만으로도 지상
    화소가 37 m 라 이 실험에 충분합니다.

왜 아폴로인가
    달 궤도 스테레오는 거의 전부 푸시브룸이라 카메라 자세 커널이 필요합니다.
    매핑 카메라는 프레임 카메라여서 두 장 사이가 행렬 하나로 기술되고,
    이 파이프라인의 정렬·삼각측량이 그대로 성립합니다. 자세한 사정은
    tools/check_real_pair.py 에 적어 두었습니다.
...
#: 정답 고도를 함께 받아 두면 복원한 기복이 자릿수까지 맞는지 볼 수 있습니다.
#: 프레임 중심은 (25.89, -6.10) 과 (25.89, -7.43) 입니다.
