"""실제로 찍은 스테레오 쌍을 이 파이프라인에 바로 넣을 수 있는지 확인합니다.

왜 이 도구가 있는가
    "렌더링 말고 실제 사진을 쓰라" 는 것이 이 실험의 가장 큰 개선점입니다.
    실제로 해 보니 걸림돌이 어디에 있는지가 분명해졌고, 그것을 말이 아니라
    **측정으로** 남겨 두려고 만들었습니다.

무엇을 확인하는가
    Kaguya(SELENE) 지형 카메라는 앞뒤로 기울어진 두 대(TC1, TC2)를 함께 실은
    스테레오 전용 카메라입니다. 수렴각이 약 30~34도로 이 실험의 조건과 비슷하고,
    자료는 공개되어 있습니다. 두 가지 형태로 받을 수 있는데 둘 다 문제가 있습니다.

    (가) USGS 가 지도 투영해 둔 COG — 받기 쉽고 두 장이 같은 격자에 놓입니다.
         그런데 **전역 DEM 을 지형 모델로 써서 정사보정**되어 있습니다. 즉 고도에
         따른 어긋남이 이미 제거되어 있어 스테레오로 쓸 수 없습니다.
         이 스크립트가 그것을 숫자로 확인합니다.

    (나) DARTS 의 원본(level 2B0) — 투영 전이라 어긋남이 살아 있습니다. 대신
         푸시브룸 카메라 모델(SPICE/ISIS 또는 CSM)이 있어야 어긋남을 고도로
         바꿀 수 있습니다. 카메라 모델 없이 특징점만으로 맞추려 해 보았으나
         실패했습니다. 그 기록은 아래 REPORT 에 적어 두었습니다.
...
#: 정사보정에 쓰인 지형 모델. 관측의 provenance.txt 에 그대로 적혀 있습니다.
...
def main() -> int:
    ...
    log(f"\n  구면에 투영했다면 고도 h 가 어긋남 d 로 남아야 합니다")
    ...
    log("  극궤도라 시차는 세로(궤도 방향)에 있어야 합니다. 가로는 등록 오차입니다.")
    ...
    log(f"  상관계수        {corr:+.3f}   (1 에 가까워야 시차입니다)")
    log(f"  맞춘 기울기     {abs(slope):.2f} m/px")
    log(f"  기하가 예측     {predicted:.2f} m/px")
    log(f"  어긋난 정도     {abs(abs(slope)/predicted - 1)*100:.0f}%")

    log("\n[3] 왜 이런가")
    log("  이 영상들의 처리 기록에 답이 있습니다.")
    log(f"    {PROVENANCE}")
    log("  전역 DEM 을 지형 모델로 놓고 정사보정한 것입니다. 고도에 따른 어긋남이")
    log("  이미 제거되어 있으므로, 남은 것은 그 DEM(512 ppd, 약 59 m/화소)보다")
    log("  잘아서 지워지지 않은 잔무늬뿐입니다. 스테레오로 쓸 수 없습니다.")

    log("\n[4] 투영 전 원본(level 2B0)은 어떤가")
    log("  DARTS 에서 받을 수 있고 크기도 25 MB 로 작습니다. 어긋남은 살아 있습니다.")
    log("  다만 푸시브룸 카메라 모델이 있어야 어긋남을 고도로 바꿀 수 있습니다.")
    log("  카메라 모델 없이 특징점만으로 맞춰 보았으나 실패했습니다.")
    for name, result in RAW_ATTEMPTS:
        log(f"    {name:16s} {result}")
    log("  티코 안쪽 벽은 태양 고도가 8도라 그림자가 지배하고, 두 카메라가")
    log("  반대쪽에서 보므로 한쪽에 보이는 사면이 다른 쪽에서는 그늘입니다.")
    log("  이 지형에서 카메라 모델 없이 맞추는 것은 무리입니다.")

    log("\n결론")
    log("  실제 스테레오 쌍은 공개되어 있고 받기도 쉽습니다. 걸림돌은 자료가 아니라")
    log("  **푸시브룸 카메라 모델을 다루는 도구 체계**(ISIS3 / CSM)다. 지도 투영된")
    log("  판본은 이미 정사보정되어 시차가 없고, 원본은 카메라 모델이 필요합니다.")
    log("  개선점 1번이 남아 있는 이유가 이것입니다.")
