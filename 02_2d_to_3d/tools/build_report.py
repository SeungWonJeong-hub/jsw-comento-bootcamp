...
def build(prs, s):
    ...
    sl = new_slide(
        ...
        f"티코 크레이터 · LOLA 레이저 고도 모델 {sc['gsd_m']/1000:.3f} km 격자 · "
        f"{sc['grid'][0]}×{sc['grid'][1]} · 달 광도함수 · SNR {sc['snr']:.0f}")
...
· 그래서 같은 지역을 각도를 바꿔 두 번 찍었습니다. 같은 지점이 두 사진에서 얼마나 밀렸는지가 곧 거리입니다. 두 눈으로 거리를 재는 것과 같습니다.

· 쓴 자료는 두 가지입니다. 지형은 LRO LOLA 레이저 고도계의 전역 고도 모델에서 티코 크레이터를 {sc['grid'][0]}×{sc['grid'][1]} 으로 잘라 썼습니다. 격자 한 칸이 {sc['gsd_m']:.0f} m 입니다.
· 표면 무늬는 같은 구역을 실제로 찍은 LRO WAC 영상에서 음영을 빼내 가져왔습니다. 고도 모델이 레이저 궤적을 편 것이라 매끈해서 잔무늬가 없기 때문입니다. 둘 다 USGS 공개 자료입니다.

· 만드는 순서는 여섯 단계입니다.
· 하나. 두 장을 만듭니다. 고도를 3차원으로 펴고 자세를 아는 카메라로 두 번 찍습니다. 밝기는 달 광도함수로 만들고 광자 잡음과 광학 흐림까지 넣었습니다.
· 둘. 두 장을 나란히 폅니다. 상대 자세로 정렬해 같은 지점이 같은 가로줄에 오게 합니다. 그래야 한 줄만 훑으면 됩니다.
· 셋. 같은 지점을 찾습니다. OpenCV 의 SGBM 을 씁니다. 가로로 두 배 늘려 맞춰서 시차를 반 픽셀 단위로 얻습니다.
· 넷. 못 믿을 곳을 버립니다. 무늬가 옅은 곳 좌우 왕복이 깨진 곳 광도 잔차가 큰 곳입니다. 네 관문 모두 정답 고도를 쓰지 않습니다.
· 다섯. Z 는 f 곱하기 B 를 d 로 나눈 값입니다. 이 한 줄이 깊이 맵입니다. 앞의 네 단계는 전부 여기 넣을 d 를 믿을 수 있게 만드는 일입니다.
· 여섯. 수렴각이 다른 네 쌍을 지형 격자 위에서 합칩니다.

· 촬영 조건은 고도 {sc['altitude_m']/1000:.0f} km 두 촬영 지점 사이 {sc['baseline_m']/1000:.1f} km 시선이 벌어진 각 {sc['convergence_deg']:.0f}도입니다.
· 여기서 시차 1픽셀이 {res/1000:.2f} km 입니다. 이보다 미세한 높낮이는 원리적으로 구분되지 않습니다.
· 결과는 Z 오차 중앙값 {best['median_abs']/1000:.3f} km 입니다. 시차로 {best['median_abs']/res:.2f} 픽셀이니 한 픽셀보다 작은 수준입니다.
""")
    ...
    add_notes(sl, f"""
· 왜 했는가. 테스트가 출력의 크기와 자료형만 보면 계산식이 틀려도 통과합니다. 그러면 있으나 마나입니다.

· 그래서 손으로 답을 낼 수 있는 조건을 만들어 숫자까지 대조했습니다.
· 평지를 수직으로 내려다보면 모든 픽셀의 깊이가 촬영 고도와 같아야 합니다. 베이스라인은 B = 2 곱하기 H 곱하기 탄젠트 수렴각 반 을 만족해야 합니다. 깊이 맵을 3차원으로 폈다가 되돌리면 원래 고도가 나와야 합니다.
· 여기에 잘못된 입력이 예외를 내는지 그리고 한 번 잡은 버그가 다시 살아나지 않는지를 더했습니다. 슬라이드에 보이는 코드가 그중 베이스라인 불변식입니다.

· 결과는 {tc}개 전부 통과입니다.
· 그리고 이 테스트가 실제로 버그를 잡았습니다. 지형을 그릴 때 점을 픽셀에 던지며 반올림했는데 그 오차가 경사 방향으로 쏠렸습니다. 두 촬영 지점이 반대로 기울어 있어 시차가 2.6 픽셀 치우쳤고 고도가 1.245 km 틀렸습니다.
· 픽셀마다 광선을 쏘는 방식으로 바꿔 0.1 픽셀로 줄였습니다.
""")
...
· 크레이터 바닥과 테두리 그리고 가운데 봉우리가 그대로 살아 있습니다.
...
· 한 쌍만 쓰면 오차 {one['median_abs']/1000:.3f} km 에 {one['coverage']*100:.0f}퍼센트고 각도를 크게 벌린 한 쌍은 {widest['median_abs']/1000:.3f} km 로 정확하지만 {widest['coverage']*100:.0f}퍼센트밖에 못 덮습니다.
...
· 복원한 높낮이 차가 {ap['relief_m']/1000:.2f} km 인데 같은 곳을 레이저로 잰 값이 2.72 km 입니다. {ap['relief_m']/2719:.2f} 배 안에서 맞습니다.
· 처음에는 고도에 중앙이 솟은 사발이 있었습니다. 스캔에 남은 렌즈 왜곡이었고 그 계수도 대응점만으로 찾아 걷어냈습니다.
""" if ap else ""))
    ...
    for num, head, now, why in items:
        ...
        y += step
    add_notes(sl, f"""
· 개선점 둘입니다. 앞줄이 왜 그런지 뒷줄이 그래서 무엇을 개선해야 하는지입니다.
· 1번입니다. 앞 장에서 실제로 찍힌 두 장으로 복원까지 했는데 견준 것은 높낮이 차와 카메라까지의 거리였습니다.
· 복원한 고도가 지도 좌표 위에 놓여 있지 않아서 정답 고도와 픽셀을 하나하나 맞대어 볼 수가 없었기 때문입니다.
· 그래서 실제 사진에서 오차가 몇 미터인지를 아직 한 숫자로 말하지 못합니다. 픽셀마다 채점할 수 있는 상태로 개선해야 할 것 같습니다.
· 2번입니다. 앞에서 크게 틀린 픽셀을 골라내 버리니 결과가 좋아졌다고 말씀드렸는데 그 신호가 답하는 질문은 하나뿐이었습니다.
· 버릴까 말까는 잘 답합니다. 크게 틀린 픽셀을 고르는 AUC 가 {best_auc['auc']:.2f} 입니다. 그런데 오차가 얼마나 큰지와는 순위 상관이 {best_signal['spearman']:+.2f} 뿐입니다.
· 신호 자체가 크게 틀린 것을 잡는 쪽에 맞춰져 있기 때문입니다.
· 착륙 지점을 고르려면 여기는 몇 미터 안이라고 말할 수 있어야 하는데 지금은 못 합니다. 크기까지 예측하는 쪽으로 개선해야 할 것 같습니다.
· 재 보기 전에는 몰랐던 것도 나왔습니다. 국소 대비는 무작위보다 나빴습니다. 대비가 높은 크레이터 테두리가 오히려 더 크게 틀리기 때문입니다.
""")
