...
""" if ap else ""))
    ...
    items = [
        ("1", "실제 사진 결과를 기복의 크기로만 견줬다",
         "복원한 고도가 지도 좌표 위에 놓여 있지 않아서, 정답 고도와 화소를 "
         "맞대어 볼 수가 없었습니다. 그래서 견준 것이 기복의 크기와 "
         "카메라까지의 거리뿐입니다.",
         "이대로는 실제 사진에서 오차가 몇 미터인지를 한 숫자로 말할 수 "
         "없습니다. 화소마다 채점할 수 있는 상태로 개선해야 할 것 같습니다."),
        ("2", "남긴 값이 얼마나 정확한지는 말하지 못한다",
         f"정답 없이 쓰는 신호가 크게 틀린 화소를 골라내는 쪽에 맞춰져 "
...
· 1번입니다. 앞 장에서 실제로 찍힌 두 장으로 복원까지 했는데, 견준 것은 기복의 크기와 카메라까지의 거리였습니다.
· 복원한 고도가 지도 좌표 위에 놓여 있지 않아서, 정답 고도와 화소를 하나하나 맞대어 볼 수가 없었기 때문입니다.
· 그래서 실제 사진에서 오차가 몇 미터인지를 아직 한 숫자로 말하지 못합니다. 화소마다 채점할 수 있는 상태로 개선해야 할 것 같습니다.
· 2번입니다. 앞에서 크게 틀린 화소를 골라내 버리니 결과가 좋아졌다고 말씀드렸는데, 그 신호가 답하는 질문은 하나뿐이었습니다.
· 버릴까 말까는 잘 답합니다. 크게 틀린 화소를 고르는 AUC 가 {best_auc['auc']:.2f} 입니다. 그런데 오차가 얼마나 큰지와는 순위 상관이 {best_signal['spearman']:+.2f} 뿐입니다.
