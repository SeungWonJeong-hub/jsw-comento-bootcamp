...
def build(prs, s):
    ...
    add_text(sl, 0.98, 5.55, 6.08, 1.10, [
        (x, MONO, 9.5, BODY) for x in (
            "① 상대 자세   R_ij = R_j·R_iᵀ,  t_ij = t_j − R_ij·t_i",
            "② 베이스라인  B = t_ij 중 화면과 나란한 성분",
            "③ 정렬·정합   stereoRectify → StereoSGBM → 밀린 거리 d",
            "④ 깊이        Z = f · B / d",
    ...
    add_matrix(sl, 7.71, 5.57, [2.80, 1.84], [
        ...
        ((f"밀린 거리 d (기대)", f"{best['expected_disparity_px']:.1f} px"), "body"),
        (("d 1픽셀이 바꾸는 깊이",
...
· 그래서 두 장면의 자세에서 상대 자세를 계산했습니다. R_ij 는 R_j 에 R_i 의 전치를 곱한 것이고, t_ij 는 t_j 에서 R_ij 와 t_i 의 곱을 뺀 것입니다.
· 그 t_ij 중 화면과 나란한 성분이 베이스라인입니다. 회전 {best['rotation_deg']:.1f}도짜리 쌍에서 {best['baseline_m']:.3f} m 가 나왔습니다.
· 이것을 stereoRectify 에 넣으면 두 사진이 평행하게 펴지고, StereoSGBM 이 같은 지점이 옆으로 얼마나 밀렸는지를 찾습니다. 기대 시차가 {best['expected_disparity_px']:.0f} 픽셀입니다.
...
· 결과는 거리 오차 중앙값 {best['median_abs']*100:.2f} cm 입니다. 5 cm 안에 드는 비율은 3장에서 보여 드리겠습니다.
""")
...
""")
    ...
    add_panel(sl, 5.26, 5.02, 7.35, 1.86,
              "거리는 정확합니다 · 남은 문제는 덮는 범위입니다", [
        tk(f"정확합니다 — 100번 중 {best['within_5cm']*100:.0f}번이 5 cm 안에 "
           f"들어옵니다."),
