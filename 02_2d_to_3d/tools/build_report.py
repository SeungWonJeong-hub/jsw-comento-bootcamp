...
· 여기서 시차 1픽셀이 {best['depth_resolution_m_per_px']*100:.1f} cm 입니다. 이보다 미세한 구조는 원리적으로 구분되지 않습니다.
· 결과는 Z 오차 중앙값 {best['median_abs']*100:.2f} cm, 위성까지 {best['distance_m']:.1f} m 인데 그 0.13퍼센트입니다.
""")
...
· 그래서 공과 상자로 정답을 손으로 풀 수 있는 장면을 만들었습니다. 정답에 오차가 없으니 틀린 만큼이 그대로 알고리즘의 오차입니다.
· 왼쪽이 실제 코드입니다. 1장에서 말한 "돌아간 각도가 두 번째 눈이 된다" 를 소수점 12자리까지 확인합니다.
· 오른쪽이 실행 결과, {tc}개 전부 통과입니다. outputs 폴더에 그대로 저장돼 있습니다.
...
· 자세를 0.25도만 틀어도 무너지는 이유는 베이스라인이 짧아서입니다. 0.25도면 타겟이 2.5 cm 옮겨 앉는데, 베이스라인 {best['baseline_m']*100:.0f} cm 의 7퍼센트라 깊이가 통째로 그만큼 밀립니다.
