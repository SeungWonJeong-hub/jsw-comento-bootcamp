...
· 두 촬영 지점은 서로 반대로 기울어 있어 좌우가 반대로 쏠렸고, 시차가 2.6 픽셀 치우쳐 고도가 1,245 m 틀렸습니다.
· 화소마다 광선을 쏘는 방식으로 바꿔 0.1 픽셀로 줄였습니다. 고도 오차는 {best['median_abs']:.0f} m 가 됐습니다.
""")
    ...
    add_panel(sl, 5.26, 5.02, 7.35, 1.86,
        ...
        t(f"한 쌍만 쓰면 {one['median_abs']:.0f} m 에 {one['coverage']*100:.0f}%, "
          f"각도를 벌린 한 쌍은 {widest['median_abs']:.0f} m 에 "
...
· 한 쌍만 쓰면 오차 {one['median_abs']:.0f} m 에 {one['coverage']*100:.0f}퍼센트, 각도를 크게 벌린 한 쌍은 {widest['median_abs']:.0f} m 로 정확하지만 {widest['coverage']*100:.0f}퍼센트밖에 못 덮습니다.
...
""")
    ...
    items = [
        ...
        ("3", "시차가 정수에 몰리는 것 더 줄이기",
         ...
         "기본 배포에는 그 모듈이 없습니다."),
        ("4", "더 촘촘한 고도 모델 쓰기",
         "지금 고도 모델은 118 m/화소이고, 레이저 궤적을 격자로 편 것이라 "
         "매끈해서 잔무늬가 적습니다.",
         "→ 2~5 m/화소 모델을 쓰면 정합이 쉬워집니다. 전용 포맷과 도구 체계가 "
         "필요합니다."),
    ]
...
· 4번은 고도 모델 해상도입니다. 지금은 118 m 화소라 매끈해서 잔무늬가 적습니다. 더 촘촘한 모델을 쓰면 정합이 쉬워지는데, 전용 포맷과 도구가 필요합니다.
""")
