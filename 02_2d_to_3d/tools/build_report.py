...
""")
    ...
    add_notes(sl, f"""
· 테스트가 출력의 크기와 자료형만 보면 계산식이 틀려도 통과합니다. 그런 테스트는 있으나 마나입니다.
· 그래서 손으로 답을 낼 수 있는 조건을 만들었습니다. 평지를 수직으로 내려다보면 모든 픽셀의 깊이가 촬영 고도와 같아야 합니다. 광축에 수직인 평면이기 때문입니다.
· 왼쪽이 그 테스트 코드입니다. 오른쪽이 실행 결과입니다. {tc}개 전부 통과입니다.
· 이 테스트들이 실제로 버그를 잡았습니다. 지형을 그릴 때 점을 픽셀에 던지면서 위치를 반올림했는데 그 오차가 경사 방향으로 쏠렸습니다.
· 두 촬영 지점은 서로 반대로 기울어 있어 좌우가 반대로 쏠렸고 시차가 2.6 픽셀 치우쳐 고도가 1.245 km 틀렸습니다.
· 픽셀마다 광선을 쏘는 방식으로 바꿔 0.1 픽셀로 줄였습니다. 고도 오차는 {best['median_abs']/1000:.3f} km 가 됐습니다.
""")
