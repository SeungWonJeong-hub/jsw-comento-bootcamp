...
""")
    ...
    if ap:
        rows += [
            (("실제 사진 — 기복", f"{ap['relief_m']/1000:.2f} km"), "key"),
            (("  레이저가 잰 기복", "2.72 km"), "body"),
    ...
    add_panel(sl, 5.26, top, 7.35, tall,
        ...
        t(f"남은 {100-fz['coverage']*100:.0f}% 는 크레이터 안쪽 그늘입니다. "
          ...
          t("1971년 아폴로 15호가 24초 간격으로 필름에 찍은 두 장을 같은 "
            "파이프라인에 넣었습니다. 복원한 기복이 레이저로 잰 기복과 "
...
· 복원한 기복이 {ap['relief_m']/1000:.2f} km 인데, 같은 지역을 레이저로 잰 기복이 2.72 km 입니다. {ap['relief_m']/2719:.2f} 배 안에서 맞습니다.
...
""" if ap else ""))
    ...
    items = [
        ("1", "실제 사진을 지도 위에 놓고 화소마다 채점하기",
         "실제로 찍힌 두 장으로 복원까지 했지만, 견준 것은 기복의 크기뿐입니다. "
         "화소마다 오차를 재지는 못했습니다.",
         "지도 좌표에 놓으려면 촬영 자세 전체가 필요합니다. 한 쌍이 아니라 "
         "여러 쌍으로 늘리는 것도 남아 있습니다."),
        ("2", "믿을 곳을 가려내는 신호가 약하다",
         f"정답 없이 계산되는 신호 셋을 넣고 재 봤더니, 가장 나은 것도 "
         f"실제 오차와의 순위 상관이 {best_signal['spearman']:+.2f} 였습니다.",
         "무작위보다는 낫지만, 화소 하나하나에 대해 여기는 믿어도 된다고 "
         "말할 수준은 아닙니다."),
        ("3", "블록 크기를 정답 없이 못 고른다",
         f"각도가 다른 두 촬영의 불일치로 골랐더니 블록 "
         f"{ap['picked_without_truth']} 를, 정답으로 고르면 "
         f"{ap['picked_with_truth']} 를 고릅니다.",
         f"블록을 키우면 두 각도가 똑같이 뭉개져 불일치가 계속 줄기 때문입니다. "
         f"그래서 {cost:.0f}% 나쁜 값을 고릅니다."),
    ...
    for num, head, now, why in items:
        ...
        y += 1.22
    add_notes(sl, f"""
· 개선점 넷입니다. 앞줄이 무엇이 부족한지, 뒷줄이 왜 그것이 문제인지입니다.
· 1번이 가장 큽니다. 밝기를 달 광도함수로 만들고 광자 잡음과 광학 흐림까지 넣었지만, 여전히 제가 고도 모델에서 만든 영상입니다.
· 실제 표면의 알베도 무늬와 거칠기는 들어 있지 않습니다. 그래서 이 숫자가 검증한 것은 촬영 기하와 정합이지 촬영 자체가 아닙니다.
· 2번은 신뢰도입니다. 실제 운용에는 정답 고도가 없으니 두 영상만 보고 어디를 믿을지 정해야 합니다.
· 좌우 일관성 검사와 광도 잔차를 넣고, 그것이 실제 오차를 예측하는지를 희소화 곡선으로 쟀습니다.
· 결과는 무작위보다는 낫지만 약했습니다. 가장 나은 것도 순위 상관 {best_signal['spearman']:+.2f} 입니다.
· 재 보기 전에는 몰랐던 것도 나왔습니다. 국소 대비는 무작위보다 나빴습니다. 대비가 높은 크레이터 테두리가 오히려 더 크게 틀리기 때문입니다.
· 3번은 파라미터입니다. 대비 하한은 영상의 잡음에서 유도하도록 바꿔서 풀렸는데, 블록 크기는 절반만 풀렸습니다.
· 각도가 다른 두 촬영의 결과가 얼마나 일치하는지로 골라 봤습니다. 정답을 안 쓰는 기준입니다.
· 그런데 블록을 키우면 두 각도가 똑같이 뭉개져서 불일치가 계속 줄어듭니다. 그래서 {cost:.0f}퍼센트 나쁜 값을 고릅니다.
· 4번은 시차가 정수 부근에 몰리는 문제입니다. 가로로 2배 늘려 정합해서 49에서 36퍼센트까지 낮췄는데, 고르게 퍼지면 20퍼센트여야 하니 아직 남아 있습니다.
""")
