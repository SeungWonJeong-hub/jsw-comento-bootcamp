...
""")
    ...
    # ---------------- 4. 개선점 ----------------
    # 검토에서 나온 것을 '이번에 고친 것' 과 '남은 것' 으로 나눠 놓는다.
    # 섞어 놓으면 무엇이 이미 반영된 값인지가 흐려진다.
    sl = new_slide(
        prs, 4, "04 / 개선점",
        "고친 것과 남은 것",
        "왼쪽은 이번 검토에서 찾아 고쳤고, 오른쪽은 아직 하지 않았습니다")
    add_image(sl, os.path.join(OUT, "01_tradeoff.png"), 0.72, 1.70, 11.89, 2.40)

    add_panel(sl, 0.72, 4.32, 5.86, 2.50, "찾아서 고친 것", [
        tk("① 렌더링이 시차를 밀어냄 — 오차 1,245 → 66 m"),
        t("　점을 화소에 던질 때의 반올림이 경사 방향으로 쏠렸습니다."),
        tk(f"② 시차가 정수에 몰림 — 오차 65 → {best['median_abs']:.0f} m"),
        t("　가로로 2배 늘려 정합하니 절반이 됐습니다."),
        tk("③ 무늬 없는 곳의 헛값 — 크게 틀린 화소 3분의 1 감소"),
        t("　이 밖에 광선 미수렴, 깊이 구간, 시점 방향까지 여섯 건."),
    ])
    add_panel(sl, 6.75, 4.32, 5.86, 2.50, "남은 것", [
        tk("① 화소마다 신뢰도를 함께 내기"),
        t("　좌우에 같은 사진을 넣어도 21%가 값을 냅니다. 매처는 탐색"),
        t("　구간 안에서 반드시 무언가를 고르기 때문입니다."),
        tk(f"② 그늘진 곳 메우기 — 값이 안 나오는 "
           f"{100 - best['valid_ratio']*100:.0f}%"),
        t("　태양 각도가 다른 시점을 더 모으면 줄일 수 있습니다."),
        tk("③ 수렴각을 용도에 맞춰 고르기 (위 왼쪽 그래프)"),
    ])
    add_notes(sl, f"""
· 검토에서 나온 것을 고친 것과 남은 것으로 나눴습니다.
· 고친 것 중 가장 컸던 둘을 말씀드리겠습니다.
· 첫째, 지형을 그릴 때 점을 화소에 던지면서 위치를 반올림했는데 그 오차가 경사 방향으로 쏠렸습니다. 두 촬영 지점이 서로 반대로 기울어 있어 좌우가 반대로 쏠렸고, 결과가 1,245 m 치우쳤습니다. 화소마다 광선을 쏘는 방식으로 바꿔 66 m 가 됐습니다.
· 둘째, 시차가 정수 부근에 몰려 있었습니다. 소수부를 세어 보니 정수 근처에 49퍼센트가 있었는데 균일하면 20퍼센트여야 합니다. 시차는 가로 방향이니 가로로만 2배 늘려 정합했더니 오차가 65에서 {best['median_abs']:.0f} m 로 거의 절반이 됐습니다.
· 이 밖에 광선이 수렴하지 않은 채 값을 내던 것, 깊이 탐색 구간을 잘못 잡던 것, 시점 방향이 뒤집히던 것까지 여섯 건을 고쳤습니다. 전부 회귀 테스트로 고정했습니다.
· 남은 것 중 첫째가 가장 큽니다. 좌우에 같은 사진을 넣어도 화소의 21퍼센트가 값을 냅니다. 매처는 탐색 구간 안에서 반드시 무언가를 고르기 때문입니다. 즉 이 파이프라인은 "대응이 없다" 를 말할 수 없습니다.
· 착륙 지점을 고르는 일이라면 모르는 곳을 피하는 것이 정확한 것만큼 중요하니, 화소마다 신뢰도를 함께 내야 합니다.
· 둘째는 그늘입니다. 값이 안 나오는 {100 - best['valid_ratio']*100:.0f}퍼센트가 대부분 크레이터 안쪽 그늘인데, 태양 각도가 다른 시점을 더 모으면 줄일 수 있습니다.
· 셋째는 위 그래프입니다. 시선을 더 벌리면 정밀해지고 빈 곳이 늘어납니다. 용도가 정해지면 다시 골라야 합니다.
""")
