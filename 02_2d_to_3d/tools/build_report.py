...
""")
    ...
    sl = new_slide(
        ...
        f"레이저로 잰 고도와 중앙값 {fz['median_abs']:.0f} m 안에서 일치합니다",
        f"복원한 점 {s['n_points']:,}개 · 정답은 LRO LOLA 레이저 고도계 측정치")
    ...
    add_matrix(sl, 0.98, 5.60, [2.34, 1.44], [
        (("고도 오차 (중앙값)", f"{fz['median_abs']:.0f} m"), "key"),
        (("90 퍼센타일", f"{fz['p90_abs']:.0f} m"), "body"),
        (("지형 기복 대비", f"{fz['median_abs']/sc['relief_m']*100:.2f}%"), "key"),
        (("값이 나온 셀", f"{fz['coverage']*100:.0f}%"), "body"),
    ])
    add_panel(sl, 5.26, 5.02, 7.35, 1.86,
              "높낮이를 살렸고, 남은 것은 그늘입니다", [
        tk(f"기복 {sc['relief_m']:,.0f} m 인 크레이터를 오차 {fz['median_abs']:.0f} m 로 "
           f"복원했습니다 — 기복의 {fz['median_abs']/sc['relief_m']*100:.2f}% 입니다."),
        t(f"사진 한 쌍만 쓰면 {one['median_abs']:.0f} m 에 "
          f"{one['coverage']*100:.0f}% 인데, 수렴각이 다른 네 쌍을 합쳐 "
          f"둘 다 좋아졌습니다."),
        t(f"대신 {100-fz['coverage']*100:.0f}% 의 셀에서는 값이 나오지 않습니다. "
          f"크레이터 안쪽 그늘입니다."),
    ])
    add_notes(sl, f"""
· 왼쪽이 정답 지형, 오른쪽이 복원한 점 {s['n_points']:,}개입니다. 색은 고도입니다.
· 크레이터 바닥과 테두리, 가운데 봉우리가 그대로 살아 있습니다.
· 숫자로는 고도 오차 중앙값 {fz['median_abs']:.0f} m 입니다. 기복이 {sc['relief_m']:,.0f} m 인 지형이니 그 {fz['median_abs']/sc['relief_m']*100:.2f}퍼센트입니다.
· 정답은 LOLA 레이저 고도계로 실제로 잰 값입니다. 제가 만든 근사가 아닙니다.
· 사진 한 쌍만 쓰면 오차 {one['median_abs']:.0f} m 에 {one['coverage']*100:.0f}퍼센트를 덮는데, 수렴각이 다른 네 쌍을 합치니 오차도 줄고 덮는 범위도 넓어졌습니다. 다음 장에서 그 이야기를 하겠습니다.
· 다만 {100-fz['coverage']*100:.0f}퍼센트의 셀에서는 값이 나오지 않았습니다. 크레이터 안쪽 그늘이라 두 사진에 같은 무늬가 안 보이는 곳입니다.
""")
    ...
    sl = new_slide(
        prs, 4, "04 / 개선점",
        "고친 것과 남은 것",
        "왼쪽은 이번 검토에서 찾아 고쳤고, 오른쪽은 아직 하지 않았습니다")
    add_image(sl, os.path.join(OUT, "03_fusion.png"), 0.72, 1.70, 11.89, 2.40)

    add_panel(sl, 0.72, 4.32, 5.86, 2.50, "찾아서 고친 것 — 일곱 건", [
        tk(f"① 여러 수렴각을 합침 — 오차 {one['median_abs']:.0f} → "
           f"{fz['median_abs']:.0f} m, 덮은 셀 {one['coverage']*100:.0f} → "
           f"{fz['coverage']*100:.0f}%"),
        t("　정밀한 각도는 좁게, 넓은 각도는 성기게 덮습니다 (위 그림)."),
        tk("② 렌더링이 시차를 밀어냄 — 오차 1,245 → 66 m"),
        tk("③ 시차가 정수에 몰림 — 가로 2배 정합으로 65 → 35 m"),
        t("　이 밖에 무늬 관문, 광선 미수렴, 깊이 구간, 시점 방향."),
    ])
    add_panel(sl, 6.75, 4.32, 5.86, 2.50, "남은 것 — 둘", [
        tk("① 픽셀 락킹을 더 줄이기"),
        t("　가로 2배로 정수 쏠림을 49 → 36%까지 낮췄지만 균일한 20%보다"),
        t("　아직 높습니다. 4배는 오히려 나빠집니다."),
        tk(f"② 그늘을 메우기 — 값이 안 나오는 {100-fz['coverage']*100:.0f}%"),
        t("　조명 조건이 다른 시점을 더 모아야 하는데, 그러려면 실제 궤도"),
        t("　영상이 필요합니다."),
    ])
    add_notes(sl, f"""
· 검토에서 나온 것을 고친 것과 남은 것으로 나눴습니다.
· 고친 것 중 가장 큰 셋을 말씀드리겠습니다.
· 첫째, 여러 수렴각을 합쳤습니다. 앞 장에서 시선을 벌리면 정밀해지고 빈 곳이 늘어난다고 했는데, 그건 한 쌍만 쓸 때 이야기였습니다.
· 각도가 다르면 정합에 성공하는 곳도 다릅니다. 위 그림에서 10도는 넓고 성기게, 30도는 좁고 촘촘하게 덮는 것이 보입니다.
· 각도마다 정렬 창 크기가 달라 깊이 맵끼리는 못 겹칩니다. 그래서 지형 격자 위에서 합쳤고, 정밀한 각도가 무거워지도록 분해능 제곱의 역수로 가중했습니다.
· 결과가 오차 {one['median_abs']:.0f}에서 {fz['median_abs']:.0f} m, 덮은 셀 {one['coverage']*100:.0f}에서 {fz['coverage']*100:.0f}퍼센트입니다. 둘 다 좋아졌습니다.
· 각도끼리 값이 어긋나는 셀은 버렸는데, 정답을 보지 않고 믿을 수 없는 곳을 가려낼 수 있는 유일한 단서였습니다.
· 둘째, 지형을 그릴 때의 반올림 오차가 경사 방향으로 쏠려 시차를 1,245 m 밀어냈습니다. 화소마다 광선을 쏘는 방식으로 바꿔 66 m 가 됐습니다.
· 셋째, 시차가 정수 부근에 몰려 있었습니다. 가로로만 2배 늘려 정합했더니 65에서 35 m 로 절반이 됐습니다.
· 이 밖에 무늬 없는 곳을 걸러내는 관문, 광선 미수렴, 깊이 탐색 구간, 시점 방향까지 일곱 건입니다. 전부 회귀 테스트로 고정했습니다.
· 남은 것은 둘입니다. 픽셀 락킹은 아직 균일한 20퍼센트보다 높고, 그늘은 조명 조건이 다른 시점이 있어야 메울 수 있습니다.
""")
