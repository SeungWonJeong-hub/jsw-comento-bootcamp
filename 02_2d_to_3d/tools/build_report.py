...
""")
    ...
    # 목적 · 과정 · 결과 세 줄만 둔다. 코드와 실행 결과를 보면서 듣는
    # 자리라 문장이 길면 화면과 말이 따로 논다. 자세한 것은 대본에서 말한다.
    add_card(sl, 0.72, 5.18, 11.89, 1.64)
    rows = [
        ("목적", "크기와 자료형만 보는 테스트는 계산식이 틀려도 통과합니다.",
         False),
        ("과정", "정답을 손으로 풀 수 있는 장면을 만들어, 숫자를 소수점 "
                 "12자리까지 대조했습니다.", False),
        ("결과", f"{test_count()}개 전부 통과. 테스트가 없던 자리에서 버그 "
                 f"4건을 찾아 고쳤습니다.", True),
    ]
    y = 5.55
    for label, text, key in rows:
        add_text(sl, 0.98, y + 0.03, 0.60, 0.22, [(label, MONO, 9, MUTED)])
        add_text(sl, 1.66, y, 10.69, 0.26,
                 [(text, SANS_MD if key else SANS, 11.5,
                   INK if key else BODY)])
        y += 0.42
    add_notes(sl, f"""
· 과제 예시 테스트는 크기와 자료형만 봅니다. 그러면 계산식이 틀려도 통과합니다.
· 목적입니다. 공과 상자로 위성을 세우면 광선과 도형의 교차를 식으로 풀 수 있어 정답에 렌더링 오차가 0 입니다. 그러면 틀린 만큼이 그대로 알고리즘의 오차가 됩니다.
· 과정입니다. 왼쪽이 실제 코드인데, 1장에서 말한 "돌아간 각도가 두 번째 눈이 된다" 를 확인합니다. 위성 위의 점 200개를 두 장면 좌표로 옮겨 놓고, 우리가 구한 R 과 t 로 한쪽을 다른 쪽에 겹쳐 소수점 12자리까지 맞는지 봅니다.
· 결과입니다. 오른쪽처럼 {tc}개 전부 통과하고, 그 파일이 outputs 폴더에 그대로 저장돼 있습니다. 정답 깊이 차이 {syn['gt_span_m']:.2f} m 를 {ss['span_m']:.2f} m 로 되살렸고, 과제 예시 코드는 {se['span_m']:.3f} m 로 납작해졌습니다.
· 이 테스트들이 버그 4건을 잡았습니다. 세로로 찍힌 쌍이 통째로 버려지고, 기준 깊이가 90도 어긋나고, 정렬 창이 타겟을 자르고, 블록 크기를 가짜 영상으로 고른 것입니다. 넷 다 예외가 나지 않아 조용히 틀리는 종류였습니다.
· 그래서 고친 뒤에는 일부러 되돌려 그 테스트가 진짜 실패하는지까지 확인했습니다.
""")
