...
def build(prs, s):
    ...
    add_notes(sl, f"""
· 사진에는 높낮이가 없어서 같은 곳을 각도를 바꿔 두 번 찍었습니다. 두 사진에서 밀린 정도가 곧 거리입니다.
· 지형은 LOLA 레이저 고도 모델의 티코 크레이터를 쓰고 표면 무늬는 같은 곳을 실제로 찍은 WAC 영상에서 가져왔습니다.
· 두 장을 나란히 펴고 SGBM 으로 시차를 찾고 못 믿을 곳을 버린 뒤 Z 는 f 곱하기 B 를 d 로 나눕니다. 마지막에 수렴각이 다른 네 쌍을 합칩니다.
...
· 오차 {fz['median_abs']/1000:.3f} km 로 높낮이 차의 {fz['median_abs']/sc['relief_m']*100:.2f}퍼센트입니다. 못 낸 {100-fz['coverage']*100:.0f}퍼센트는 크레이터 안쪽 그늘입니다.
· 맨 오른쪽은 1971년 아폴로 15호가 실제로 찍은 두 장입니다. 사진에서 잰 것만으로 계산한 카메라 고도와 복원한 높낮이 차가 기록된 값과 맞습니다.
""")
