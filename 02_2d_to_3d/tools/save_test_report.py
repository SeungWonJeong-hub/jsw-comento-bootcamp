"""Unit Test 를 실행하고 결과를 outputs/pytest_report.txt 로 남깁니다.

왜 따로 두는가
    과제 요청은 "Unit Test 코드 및 실행 결과 문서화" 다. 실행 결과가 화면에만
    남으면 문서가 아니므로 파일로 남기고, 발표자료도 이 파일을 읽어 씁니다.

    pytest 를 그냥 리다이렉트하면 마지막 줄에 걸린 시간이 들어가 실행할 때마다
    파일이 달라집니다. outputs/ 의 다른 산출물은 전부 바이트 단위로 재현되는데
    이 파일만 매번 바뀌면 "재현된다" 는 말이 반만 맞게 됩니다. 걸린 시간은
    측정값이 아니라 실행 환경이므로 지운 뒤 저장합니다.
...
def main() -> int:
    ...
    text = proc.stdout
    # 걸린 시간과 루트 경로를 지웁니다. 둘 다 실행 환경이지 측정값이 아닙니다.
