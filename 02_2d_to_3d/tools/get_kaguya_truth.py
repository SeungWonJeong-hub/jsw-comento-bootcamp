"""실제 스테레오 쌍과 같은 격자에 LOLA 정답 고도를 얹는다.

왜 따로 받는가
    Kaguya 영상은 IAU_2015:30115 로, LOLA 전역 모델은 단순원통도법으로
    저장돼 있다. 좌표계가 다르면 화소끼리 견줄 수 없다. LOLA 쪽을 영상과
    **같은 격자로 다시 샘플링**해 두어야 오차를 화소 단위로 잴 수 있다.

    LOLA 는 118 m/화소이고 영상은 5 m/화소다. 늘려서 얹는 것이므로 정답의
    실제 해상도가 올라가지는 않는다. 큰 지형의 높낮이를 맞추는지 보는 용도다.
