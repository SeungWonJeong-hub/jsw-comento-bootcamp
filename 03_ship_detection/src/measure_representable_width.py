...
물음
----
회전 박스를 쓰려면 장변과 단변이 둘 다 있어야 합니다. 10 m 해상도에서
한국 선박의 단변(폭)이 몇 픽셀인지 재면, 박스 규약 논쟁이 끝납니다.

왜 이 물음이 규약 논쟁을 끝내나
--------------------------------
GFW 는 폭을 주지 않습니다. 길이와 방향만 줍니다. 그래서 나는 폭을
width_ratio 로 만들어 냈고, 그 값이 정답 노릇을 했습니다. 그런데 어떤 비율을
고르는 것이 옳은가를 따지기 전에, 애초에 폭이 화소로 표현되는지를
먼저 물어야 했습니다. 표현되지 않으면 어떤 비율도 옳지 않습니다.

실제 선박의 폭/길이는 대략 0.12~0.25 범위입니다. 상선일수록 작고 어선일수록
큽니다. 이 범위 전체를 넣어 보고, 결과가 범위와 무관하게 같은 결론이면
비율을 고를 필요 자체가 사라집니다.
...
# 실제 선박의 폭/길이 범위. 상선 쪽이 작고 어선 쪽이 큽니다.
...
def main():
    ...
    print(f"\n폭이 몇 픽셀이 되나 — GFW 는 폭을 주지 않으므로 비율을 가정합니다")
    ...
    print(f"  가장 관대한 비율 {worst:.2f} 로 잡아도 "
          f"{100 * frac1:.1f}% 가 폭 1 픽셀 미만입니다.")
    print(f"  선박의 폭은 이 해상도에 물리적으로 존재하지 않습니다.")
    print(f"  따라서 회전 박스의 단변은 어떤 규약을 골라도 허구입니다.")
    print(f"  측정 가능한 것은 중심점, 길이, 방향 셋뿐입니다.")
    print(f"  평가는 점 기반으로 합니다. IoU 는 없는 축을 재게 됩니다.")
    print(f"  (xView3-SAR 이 10 m SAR 에서 점 기반 F1 을 쓴 이유와 같습니다)")
    print(f"\n  더 높은 해상도의 정답을 구해도 이 결론은 바뀌지 않습니다.")
    print(f"  0.5 m 라벨에서 실측 폭을 얻어도 10 m 로 내리면 다시 서브픽셀입니다.")
    print(f"  고해상도 정답의 값어치는 박스를 가능하게 하는 것이 아니라")
    print(f"  이 숫자를 확인해 주는 데 있습니다.")
