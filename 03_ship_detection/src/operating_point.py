...
왜 필요한가
-----------
층화 평가에서 오탐이 test 2,353건 / val 6,082건 나왔습니다. 인스턴스가 각각 366, 349개인데
오탐이 6~17뱁니다. 그런데 이 값은 신뢰도 하한을 0.001 로 둔 것입니다.
PR 곡선을 끝까지 그리려면 낮게 둬야 해서 그런 것이지, 운용 설정이 아닙니다.
...
재학습이 필요 없습니다. 이미 있는 가중치로 예측 한 번만 하면 됩니다.
"""
...
def main():
    ...
    # 신뢰도를 아주 낮게 두고 한 번만 예측한 뒤, 임계값은 나중에 걸러서 씁니다
    recs = []                       # (신뢰도, 정탐인가)
