...
왜 IoU 대신 점인가
------------------
한국 근해 선박은 길이 중앙값 30 m = 10 m 해상도에서 3 픽셀입니다.
3 픽셀짜리 물체에 IoU 0.75 를 요구하는 것은 부분 픽셀 정확도를 요구하는 것과 같습니다.

더 중요한 이유가 있습니다. GFW 정답은 애초에 박스가 아닙니다.
  점(lat, lon) + 길이(m) + 방향(도)
회전 박스는 내가 그걸로 합성한 것입니다. 그러면 IoU 오차에 두 가지가 섞입니다.

  IoU 오차 = 모델의 위치 오차 + 내가 만든 박스의 오차

3 픽셀 물체에서는 뒷항이 앞항만큼 큽니다. 즉 mAP50-95 는 모델이 아니라
내 박스 합성을 재고 있습니다. 점 기반 평가는 그 교란을 통째로 제거합니다.

무엇을 재는가
------------
예측 박스의 중심과 정답 점 사이 거리가 허용 반경 안이면 맞은 것으로 봅니다.
허용 반경은 고정값(픽셀)과 선박 길이 비례 중 큰 쪽을 씁니다.
  - 고정값: 아주 작은 배까지 최소한의 여유를 줍니다
  - 길이 비례: 큰 배는 중심이 좀 더 떨어져도 같은 뱁니다

정답 하나에 예측 하나만 붙입니다(헝가리안 대신 신뢰도 순 탐욕 매칭).
탐지 과제에서는 순위가 곧 신뢰도이므로 이게 관례에 맞습니다.
"""
...
def greedy_match(preds, gts, radius_fn):
    """신뢰도 높은 예측부터 가장 가까운 미사용 정답에 붙입니다."""
    used = set()
...
def main():
    ...
    for b in BINS:
        ...
        # 구간 재현율과 중심오차만 본다 (AP 는 전체값을 대표로 씁니다)
