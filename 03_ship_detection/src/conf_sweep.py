...
왜
--
한국 항만 결과를 눈으로 보니 항적이 뚜렷한 배가 박스 없이 지나간다.
특히 인천이 심하다. 모델이 못 보는 것인지, 보고도 점수가 낮아 잘린
것인지를 가른다. 후자면 임계값 한 줄로 해결된다.

임계값만 낮추면 갯벌에 오탐이 붙는다. 밝고 얼룩덜룩해서 배처럼 보인다.
그래서 낮춘 다음 물 위에 있는 것만 남긴다. 두 조치는 짝이다.

  임계값을 낮춘다   -> 놓친 배를 되찾는다 (재현율)
  물 게이트를 건다  -> 새로 생긴 육지 오탐을 버린다 (정밀도)

같은 장면을 한 번만 받아 임계값만 바꿔가며 돌린다. 장면이 달라지면
비교가 안 되기 때문이다.
...
def water_gate(gray, dets, on_water):
    """탐지 중 주변이 물처럼 보이는 것만 남긴다. 판정 불가는 살려 둔다."""
    keep, drop = [], []
    for poly, cf in dets:
...
def main():
    ...
    for c in sorted(a.confs, reverse=True):
        ...
        if drop:
            # 버린 것을 빨강으로 겹쳐 그려, 게이트가 무엇을 지웠는지 본다
            v = draw(bgr, keep)
