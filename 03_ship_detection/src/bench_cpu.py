...
왜 CPU 인가
-----------
4차는 웹앱입니다. 항구를 고르면 그 자리에서 탐지가 돌아야 합니다. GPU 를 붙인
서버를 항상 켜 두는 것은 개인 과제에서 현실적이지 않습니다. 그래서 CPU 만으로
쓸 만한 지연시간이 나오는지를 먼저 재고, 그 수치 위에서 4차를 설계합니다.

왜 프로세스를 나누나
--------------------
처음에는 한 프로세스에서 PyTorch 와 ONNX Runtime 을 차례로 쟀습니다. 그랬더니
같은 스크립트를 두 번 돌렸을 때 ONNX 8 스레드가 128.0 ms 와 25.9 ms 로
5 배 달라졌습니다. 노이즈가 아니라 계통 오차입니다.

  두 런타임이 각자 스레드 풀을 만듭니다. 먼저 뜬 쪽이 코어를 잡고 있으면
  나중 쪽은 남은 코어를 두고 경합합니다. 18 코어에 torch 가 14 개를 잡은
  상태에서 ORT 가 8 개를 더 요구하면 초과 구독이 됩니다.

그래서 설정 하나당 프로세스를 새로 띄웁니다. 각 프로세스는 자기 런타임만
올립니다. 느리지만 이 수치는 믿을 수 있습니다.

무엇을 재나
-----------
  1. 타일 한 장(320x320) 추론 시간 — PyTorch 대 ONNX Runtime
  2. 스레드 수에 따른 변화 — 기본값이 최적인 경우가 드물습니다
  3. 항만 한 곳 전체 주사 시간 — 실제로 웹앱이 기다리게 할 시간

3번이 사용자가 체감하는 값입니다. 1번만 재고 "빠르다" 고 말하면 안 됩니다.
항만 사각형은 타일 수십 장으로 쪼개지기 때문입니다.
...
def run_one(kind, path, imgsz, threads, runs):
    """설정 하나만 재고 JSON 한 줄을 찍습니다. 부모가 이것을 모읍니다."""
    ...
    if kind == "torch":
        ...
        # 비교하면 안 됩니다. 런타임 자체의 상한을 보는 용도입니다.
        ...
        # 전처리와 NMS 를 포함한 끝에서 끝까지. 웹앱이 실제로 쓸 값입니다.
...
def main():
    ...
    print(f"설정마다 프로세스를 새로 띄우고 {a.repeat} 번 반복합니다\n")

    # ONNX 를 먼저 만들어 둡니다. 벤치 프로세스 안에서 내보내면 그 프로세스만
    # 조건이 달라집니다.
    ...
    # --- 항만 한 곳 전체 주사 ---
    # 끝에서 끝까지 잰 값만 씁니다. session.run 값을 쓰면 NMS 를 공짜로 치는 셈입니다.
    ...
    if PORTS:
        ...
        print("웹앱에서 기다릴 만합니다. GPU 없이 4차를 설계할 수 있습니다."
              if med < 5 else "체감이 길습니다. 타일을 줄이거나 더 작은 모델을 씁니다.")
