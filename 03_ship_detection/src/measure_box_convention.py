...
왜 만들었나
-----------
YOLO-OBB 는 여덟 개 좌표로 배포됩니다. 그래서 나는 이 데이터가 회전 박스라고
믿고 학습·평가를 짰습니다. 그런데 좌표를 직접 찍어 보니 그렇지 않았습니다.

  핀란드 S2Ships    13,069개 중 100.00% 가 축 정렬 (회전이 전혀 없음)
  GFW 합성 라벨     16,929개 중  0.20% 만 축 정렬 (0~90도 균일 분포)

학습은 회전이 없는 라벨로, 평가는 회전이 균일한 정답으로 했습니다.
IoU 가 0 에 가까웠던 것은 모델이 못해서가 아니라 규약이 달라서입니다.
형식(8좌표)과 규약(실제 각도 분포)은 다릅니다. 믿지 말고 재야 합니다.

무엇을 재나
-----------
  1. 축 정렬 비율 — 회전 박스인 척하는 HBB 를 잡아냅니다
  2. 장변 각도 분포 — {0, 90} 에 몰려 있으면 회전을 못 배웁니다
  3. 크기 구간별 종횡비 — 커질수록 얇아지면 라벨러의 붓 자국이지 배 모양이 아닙니다

3번이 왜 판별이 되나
--------------------
실제 선박의 폭/길이는 0.1~0.25 이고 크기와 무관합니다. 파나막스든 어선이든
비율은 비슷합니다. 그런데 측정값이 크기에 따라 단조 감소하면, 작은 배를
뭉툭하게 칠했다는 뜻입니다. 즉 물리가 아니라 주석 습관입니다.
...
def main():
    ...
    if len(keys) >= 3:
        ...
        if first - last > 0.15:
            print(f"\n판정: 종횡비가 {first:.3f} -> {last:.3f} 로 단조 감소합니다.")
            print("      실제 선박 비율은 크기와 무관하므로, 이것은 배의 모양이")
            print("      아니라 주석 습관입니다. 작은 배를 뭉툭하게 칠한 결과입니다.")

    if near_axis > 0.9:
        print(f"\n판정: {100 * near_axis:.1f}% 가 축에 붙어 있습니다. 8좌표 OBB 형식이지만")
        print("      실질은 HBB 다. 이 라벨로 학습한 모델은 회전을 배우지 못합니다.")
        print("      회전된 정답과 IoU 로 비교하면 원리적으로 무너집니다.")
