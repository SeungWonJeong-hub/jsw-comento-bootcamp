...
만드는 것인데, **그대로는 안 된다**는 것을 3차에서 먼저 재 뒀습니다.

### 그대로 가져가면 어떻게 되는가 — 3차에서 먼저 쟀습니다

계획을 세우기 전에 돌려 봤습니다. 핀란드 데이터로 학습한 모델을 **아무것도 고치지
않고** 한국 항만 22곳에 적용하고, Global Fishing Watch 의 같은 장면 탐지와 좌표로
맞댔습니다. GFW 도 사람이 그린 정답은 아니지만 절반 이상이 AIS 로 대조돼 있어
독립적인 참조로 쓸 수 있습니다.

**결과는 나빴고, 그것이 4차의 출발점입니다.**

#### 부산 외항 정박지 — 가장 조건이 좋은 곳에서 67척 중 36척

`S2A_52SED_20221011` 장면, 신뢰도 0.05 · NMS 0.1 · 물 게이트입니다.

| 창 | 넓이 | GFW | 탐지 | 일치 | 중심 오차 |
|---|---|---|---|---|---|
| 정박지 전체 | 14.4 x 8.1 km | 67 | 70 | **36** | 0.88 px |
| 탐지가 몰린 창 | 6.4 x 3.6 km | 38 | 41 | **29** | 0.76 px |

**척수는 맞는데 짝이 안 맞습니다.** 70척을 찾아 GFW 67건과 비교했으니 총량은 거의
같지만, 실제로 같은 배를 가리키는 것은 절반입니다. 배가 몰린 자리로 창을 좁히면
29/38 로 올라갑니다 — 넓은 창에 함께 들어온 연안·부두 쪽에서 양쪽이 어긋납니다.

맞은 것의 중심 오차는 0.88 px 로 핀란드(0.46 px)와 같은 수준입니다.
**위치를 못 맞히는 것이 아니라 같은 배를 못 고릅니다.**

다른 장면·다른 임계값으로 4곳을 정량 평가한 결과는 더 낮습니다
(2022년 장면, 신뢰도 0.3, 허용 반경 5 px — `outputs/korea_quant.json`).

| 항만 | GFW | 탐지 | 정밀도 | 재현율 | **F1** | 중심 오차 |
|---|---|---|---|---|---|---|
| 부산 외항 정박지 | 58 | 47 | 0.596 | 0.483 | **0.533** | 1.89 px |
| 여수·광양만 | 49 | 14 | 0.357 | 0.102 | **0.159** | 1.77 px |
| 광양항 | 36 | 26 | 0.154 | 0.111 | **0.129** | 2.63 px |
| 통영항 | 23 | 125 | 0.072 | 0.391 | **0.122** | 3.90 px |

핀란드 평가셋의 점 기반 AP 0.911 · 재현율 0.992(5절) 옆에 놓으면 격차가
분명합니다. **해역이 바뀌면 이 모델은 그대로는 안 됩니다.** 3차의 0.9014 는
발트해 안에서의 숫자입니다.

#### 인천 — 서해 탁도에서 신뢰도가 무너집니다

인천은 물이 황토빛이고 갯벌이 넓습니다. 발트해의 "어두운 물 위 밝은 배" 라는
전제가 성립하지 않습니다. 증상은 신뢰도 분포로 나타납니다
(`S2C_52SBG_20260516` 장면, 임계값만 바꿔 가며 · NMS 0.5).

| 신뢰도 | 원본 탐지 | 물 위 | 게이트가 버림 |
|---|---|---|---|
| 0.25 | 27 | 25 | 2 (7%) |
| 0.15 | 31 | 27 | 4 (13%) |
| 0.10 | 39 | 31 | 8 (21%) |
...
**배를 못 보는 것이 아니라 자신 없게 봅니다.** 신뢰도 0.25 에서 27척뿐인데
0.03 까지 내리면 75척이 나옵니다. 항적이 뚜렷한 배가 박스 없이 지나가던 것이
이것이었습니다. 그런데 회수하면 갯벌 오탐도 같이 늘어(게이트가 버리는 비율
7% → 35%) **임계값 하나로는 못 가릅니다.**

이것이 **NIR(B08) 을 4번째 채널로 넣어야 하는 이유**입니다. 탁한 물도 근적외에서는
어둡습니다(NDWI 가 작동하는 원리). 지금은 TCI 3채널만 쓰고 있어 서해에서는
물과 배의 대비 자체가 만들어지지 않습니다.

#### 통영 — 가두리 양식장은 후처리로 못 고칩니다

통영은 반대 방향의 실패입니다. 탐지가 **395척** 나오는데(신뢰도 0.05 · NMS 0.1 ·
물 게이트, `outputs/korea/summary.json`) GFW 는 23건입니다. 다도해 가두리 양식장이
배처럼 보입니다. 처방 세 가지를 시험했고 다 실패했습니다
(아래는 같은 장면에서 임계값만 바꾼 것 · NMS 0.5).
...
**신뢰도 0.60 에서도 130척이 남습니다** — 임계값으로 안 갈립니다. 양식 시설은
물 위에 있으니 물 게이트도 통과합니다(0.60 에서 92% 가 살아남습니다). 밀집도로
거르는 것도 안 됩니다 — 정박지의 배도 똑같이 밀집합니다.

후처리로는 안 되는 종류입니다. **양식 시설을 학습 데이터에 넣어 "배가 아니다" 를
배우게 하는 것이 정공법**이고, 아래 4차 작업 목록의 "한국 해역 데이터 보강" 이
이것입니다.

> 예상은 빗나갔습니다. 처음에는 완도 양식장에서 오탐이 쏟아질 것으로 봤는데 완도는
> 오히려 조용했고(원본 93 → 게이트 통과 21척, 77% 를 버렸습니다),
> 통영이 터졌습니다. 양식 시설의 밝기가 해역마다 다릅니다.
