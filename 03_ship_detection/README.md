...
| 항만 | 창 크기 | 타일 | 추론 |
|---|---|---|---|
| 부산 북항 | 1232×1110 | 25 | 1.34 초 |
| 부산 외항 정박지 | 2112×1554 | 54 | 2.90 초 |
| 인천항 | 1584×1554 | 36 | 1.93 초 |
| 광양항 | 1583×1331 | 30 | 1.61 초 |
...
- **항만 22곳 파이프라인** — `korea_ports.py` 가 "항만 이름 → 최신 무운 장면
  검색 → 수신 → 겹쳐 탐지 → NMS" 를 한 함수로 합니다. 드롭다운이 됩니다.
- **CPU 실측** — 항만 한 곳 1.61 초 (8절). GPU 서버가 필요 없습니다.
- **참조 자료** — Global Fishing Watch 가 Sentinel-2 10 m 전 지구 탐지를
  공개합니다. 한국 근해 443,344건이고 57.1% 가 AIS 로 대조돼 있습니다. 사람이
  그린 정답은 아니지만 **독립적인 참조**로 쓸 수 있습니다.

### 미리 확인한 실패

핀란드 모델을 한국 22곳에 그대로 돌려 봤습니다. 대부분은 안정적인데
**가두리 양식장 해역에서 무너집니다.**

| 항만 | 탐지 | 판정 |
|---|---|---|
| 부산 북항 · 광양 · 울산 · 군산 | 36~145척 | 안정 |
| 인천항 | 42척 | 탁수에서도 동작 (임계값 0.05 + 물 게이트) |
| **통영항** | **395척** | **양식장 오탐** |
| 완도·다도해 | 93 → 21척 | 같은 문제 |

세 가지 처방을 시험했고 전부 실패했습니다.

| 방법 | 결과 |
|---|---|
| 임계값 상향 | `0.05→0.60` 에서 572→130척. 여전히 과다 |
| 물 게이트 | 양식장도 물 위. 해당 없음 |
| 밀집도 필터 | 통영 21% vs 마산 33%. 분리 안 됨 |

**후처리로는 못 고칩니다.** 발트해에는 가두리가 없어 모델이 배운 적이 없습니다.
학습 데이터에 넣는 것이 정공법입니다.

### 4차 작업 목록

- **양식장 하드 네거티브** — 통영·완도 해역 타일을 배경으로 라벨해 재학습
- **서해 탁도 — NIR(B08) 4번째 채널** — 탁한 물도 근적외에서는 어둡습니다
  (NDWI 가 작동하는 원리)
- **P2 헤드 (stride 4)** — 3~5 px 물체에 stride 8 격자는 거칩니다
- **항만별 임계값** — 6절에서 해역마다 최적 운용점이 다름을 측정했습니다
- **웹앱 배포** — Vercel 서버리스는 번들 250 MB 제한이라 PyTorch 를 못 싣습니다.
  ONNX Runtime 을 직접 호출하는 경로 (8절) 가 현실적입니다.
