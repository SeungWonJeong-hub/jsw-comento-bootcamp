...
(4) 를 고치려면 "옳은 종횡비" 를 정해야 합니다. 그런데 정하기 전에 먼저
물었어야 할 것이 있었습니다. **10 m 영상에 선박의 폭이 화소로 존재하는가.**

GFW 한국 근해 탐지 259,422건으로 쟀습니다. 길이 중앙값은 34.7 m,
즉 **3.47 px** 입니다. GFW 는 폭을 주지 않으므로 실제 선박이 가질 수 있는
비율 전 범위를 넣어 봤습니다.

| 가정한 W/L | 폭 중앙값 | 2 px 미만 | 1 px 미만 |
|---|---|---|---|
| 0.12 (상선) | 0.42 px | 91.8% | **79.0%** |
| 0.15 | 0.52 px | 89.4% | **74.0%** |
| 0.20 | 0.69 px | 83.5% | **67.1%** |
| 0.25 (어선) | 0.87 px | 78.0% | **58.6%** |

**어떤 비율을 골라도 폭이 1 픽셀 미만입니다.** 즉 규약을 무엇으로 정하느냐의
문제가 아니라, **잴 대상 자체가 없습니다.** 제가 쓴 `min_w=2.0` 은 물리적
폭의 3~5 배였습니다. 임의의 상수였을 뿐 아니라 방향까지 틀렸습니다.
...
3. **고해상도 정답도 이것을 못 고칩니다.** AI Hub 0.5 m 에서 실측 폭을 얻어도
   10 m 로 내리면 다시 서브픽셀입니다. 고해상도 라벨의 값어치는 박스를
   가능하게 하는 데 있지 않고 **이 숫자를 확인해 주는 데** 있습니다.
...
**4차 웹앱에 대한 함의** — 탐지 결과를 회전 박스로 그리면 안 됩니다. 중심점을
찍고 길이·방향을 함께 주는 것이 데이터가 실제로 담고 있는 것입니다.

## 9. CPU 만으로 되는가 — 4차의 전제
