...
1) 데이터: 핀란드(맑은 발트해, 사람이 그린 박스) -> GFW(황해·동중국해, AIS 대조)
   한국에서 인천의 배를 놓친 원인이 탁도였고, 그 조건이 학습에 들어갑니다.
   박스는 길이·방향 실측으로 계산합니다. 폴리곤에서 추정하지 않습니다.

2) 구조: P2(stride 4) 헤드 추가
   한국 근해 선박은 길이 중앙 30 m = 3 픽셀. stride 8 격자보다 작아
   위치 오차가 원리적으로 ±4 px 까지 벌어집니다. 격자를 절반으로 줄입니다.

평가
----
학습은 황해·동중국해, 평가는 한국. 학습에 안 쓴 한국 장면으로만 잽니다.
지표는 IoU 기반과 점 기반을 함께 봅니다. 3 픽셀 물체에 IoU 0.75 를 요구하는 것은
지표의 함정이고, GFW 정답이 원래 점이라 점 기반이 정답의 성격에 맞습니다.
"""
...
@stage("데이터 구축")
def _():
    """타일 2.1GB 를 올리는 대신, 장면 목록(1.6MB)만 받아 커널이 사진을 내려 만듭니다.

    scenes.json 에 장면별 COG 주소와 탐지 좌표·길이·방향이 들어 있습니다.
    사진은 AWS 공개 COG 라 인증이 필요 없습니다.
    """
    sh("pip install -q ultralytics rasterio")
    # 마운트 경로가 슬러그와 다를 수 있으므로 input 아래를 훑습니다
    src = None
...
# 두 축을 각각 재야 무엇이 효과였는지 귀속됩니다.
#   데이터 효과  = 표준 구조로 GFW 학습  vs  기존 핀란드 결과
#   구조 효과    = 같은 GFW 데이터에서 P2 유무
# 1차 시도에서 640 이 276분을 먹어 12시간 한도에 몰렸습니다.
# 이번에는 320 두 종만 돌려 결과를 확실히 받습니다. 640 은 필요하면 따로 돌립니다.
EXPS = [
    ("gfw_std", "yolo11s-obb.pt", 320, {}),      # GFW + 표준 구조 (데이터 효과)
    ("gfw_p2",  P2,               320, {}),      # GFW + P2       (구조 효과)
]
