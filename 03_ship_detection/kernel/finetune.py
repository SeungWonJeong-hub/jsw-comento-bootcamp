...
1차 학습에서 나온 진단
----------------------
  mAP50 0.863  vs  mAP50-95 0.459   ← 격차가 매우 크다
  평균 IoU 0.672 ~ 0.762 (크기와 함께 단조 증가)

박스가 헐겁게 맞고 있다는 뜻이다. 선박이 3~5 px 이라 한 픽셀만 어긋나도
IoU 가 20~30% 흔들린다. 그래서 IoU 문턱을 올리면 대부분 탈락한다.

처방
----
  1) 입력 해상도를 올린다 (320 -> 640).
     화소 정보가 늘지는 않지만, 탐지기의 stride 구조가 작은 물체를 더 잘 나눈다.
     소형 객체 탐지에서 표준적으로 쓰는 처방이다.
  2) 모델을 키운다 (s -> m).
  3) 더 오래 학습한다 (100 -> 200) — 덜 학습된 것이 원인인지 가른다.

셋을 나눠 돌려야 무엇이 효과였는지 귀속된다.
"""
...
EXPS = [
    # 처방 1 — 해상도. 1차 최고 모델에서 이어서 학습한다.
    ("ft_640",     BASE,             640, 50,  dict(lr0=0.002)),
    # 처방 2 — 모델 크기. 조건을 1차와 같게 두어 s 와 직접 비교한다.
    ("yolo11m_320", "yolo11m-obb.pt", 320, 100, {}),
    # 처방 3 — 학습량. 덜 학습된 것이 원인인지 가른다.
...
for name, wts, sz, ep, extra in EXPS:
    ...
    try:
        t = time.time()
        m = YOLO(wts)
        bs = 32 if sz <= 320 else 12          # 640 은 메모리가 4배라 배치를 줄인다
