...
이 모듈이 필요한 이유
    ...
    투영, 역투영, 복셀 카빙 같은 기하 연산이 '수식대로' 맞는지 검증할 근거가
    없다. 여기서는 구와 직육면체처럼 답을 손으로 풀 수 있는 도형만 써서
    실루엣과 깊이를 해석적으로 만든다. 정답에 렌더링 오차가 섞이지 않으므로
    1e-9 수준의 엄격한 단위 테스트가 가능하다.

구가 특히 유용한 이유
    구는 어느 방향에서 봐도 실루엣이 원이고, 여러 시점의 실루엣 교집합
    (visual hull)이 구 자신과 정확히 같다. 따라서 카빙 결과의 반지름이
    참값으로 수렴하는지를 오차 한계까지 계산해 확인할 수 있다.
"""
