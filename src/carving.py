...
왜 SPE3R 에 이 방법인가
    SPE3R 은 카메라가 옆으로 이동하지 않아(r = (0, 0, Z)) 스테레오 베이스라인이
    없다. 대신 타겟이 매 프레임 무작위로 회전해 시선 방향이 SO(3) 를 고르게 덮는다.
    이는 스테레오에는 최악이지만 실루엣 기반 복원에는 이상적인 조건이다.
    또한 특징점 매칭이 필요 없어 조명 변화와 무텍스처 표면에 강건하다.
