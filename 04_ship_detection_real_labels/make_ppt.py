...
def slide1(prs):
    sl = base(prs, "01 / 데이터셋", "정답이 있는 위성사진을 구했습니다 — 미국 군항 5곳",
              "HRSC2016: 구글어스에서 모은 위성사진 1,059장. 배 2,964척마다 사람이 직접 상자를 그려 둔 공개 자료입니다.", 1)
    card(sl, .72, 1.80, 7.35, 4.85)
    pic(sl, "fig2_port_spotcheck.png", .90, 1.98, w=7.0)
    txt(sl, .90, 6.20, 7.0, .4, "노란 상자가 사람이 그린 정답. 샌디에이고 · 노퍽 · 메이포트 · 에버렛 · 뉴포트 해군기지.",
        SANS, 8.5, MUTE)

    card(sl, 8.30, 1.80, 4.31, 1.5)
    txt(sl, 8.50, 1.95, 3.9, .22, "왜 이 자료인가", SEMI, 11, INK)
    txt(sl, 8.50, 2.25, 3.95, 1.0,
        "정답이 없으면 '몇 개 맞혔는지' 를 잴 수 없습니다.
"
        "그래서 사람이 정답을 그려 둔 공개 자료를 골랐습니다.",
        SANS, 9.0, BODY, line=1.45)

    card(sl, 8.30, 3.45, 4.31, 1.6)
    txt(sl, 8.50, 3.60, 3.9, .22, "어느 항구인지 직접 알아냈습니다", SEMI, 11, INK)
    txt(sl, 8.50, 3.90, 3.95, 1.1,
        "자료에는 항구 이름이 없고 위도·경도만 있습니다.
"
        "좌표를 지도에 찍어 보니 미국 해군기지 5곳이었고,
사진을 눈으로 봐서 확인했습니다.",
        SANS, 9.0, BODY, line=1.45)

    card(sl, 8.30, 5.20, 4.31, 1.45)
    txt(sl, 8.50, 5.35, 3.9, .22, "사진 한 칸이 땅에서 몇 m 인지 확인", SEMI, 11, INK)
    txt(sl, 8.50, 5.65, 3.95, .95,
        "자료에 적힌 값(1.07 m)대로면 배가 360 m — 말이 안 됩니다.
"
        "다시 계산한 0.45 m 로 재면 가장 긴 배가 항공모함 길이(333 m)와 맞습니다.",
        SANS, 9.0, BODY, line=1.45)
    return sl
...
사진마다 찾은 배와 정답을 겹쳐 보여줍니다.")
    ...
    txt(sl, .92, 4.50, 5.4, .22, "채점은 학습에 안 쓴 사진으로만", SEMI, 11, INK)
    txt(sl, .92, 4.80, 5.4, 1.8,
        "학습에 쓴 사진으로 채점하면 답을 외운 것과 구분이 안 됩니다.
"
        "그래서 처음부터 451장을 따로 빼 두고 그것으로만 점수를 냈습니다.
"
        "항구를 통째로 빼고 채점해도 결과가 같은지도 확인했습니다.",
        SANS, 9.0, BODY, line=1.5)
    card(sl, 6.75, 4.35, 5.86, 2.3)
    txt(sl, 6.95, 4.50, 5.4, .22, "상자를 기울여서 그립니다", SEMI, 11, INK)
    txt(sl, 6.95, 4.80, 5.5, 1.8,
        "군함은 부두에 비스듬히 대어 있습니다. 똑바른 네모로 감싸면 옆 배와
"
        "부두까지 들어가서 길이를 못 잽니다. 배 방향대로 기울인 상자를 쓰면
"
        "배만 딱 감싸고, 길이·폭·방향까지 나옵니다.",
        SANS, 9.0, BODY, line=1.5)
    return sl
...
"
    ...
    ports = [("fig1_san_diego.png", "샌디에이고  11척 중 12개 찾음"), ("fig1_norfolk.png", "노퍽  7척 중 7개"),
             ("fig1_mayport.png", "메이포트  6척 중 5개"), ("fig1_everett.png", "에버렛  4척 중 5개"),
             ("fig1_newport.png", "뉴포트  3척 중 1개")]
    ...
    for k, (f, cap) in enumerate(ports):
        ...
        txt(sl, x + .08, y + ch - .02, cw - .16, .2, cap, MED, 8.0, INK)
    return sl
